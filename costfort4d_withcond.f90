...
contains
  ...
  function cost4d(maxw3, maxw2, maxw1, maxw0, datasize3, datasize2, datasize1, datasize0,& 
      usecond, condparam, data_array, condition) bind(C, name="cost4d")
    ...
    real(c_double), intent(in) :: condparam
    type(result) :: cost4d                                  
    ...
    call help0d(data_array, condition, usecond, condparam)
    print *, "entering help1d"
    do ax_id1 = 1, 4
      call help1d(data_array, ax_id1, condition, usecond, condparam)
    enddo
    ...
    do ax_id2 = ax_id1 + 1, 4
      ...
      call help2d(data_array, [ax_id1, ax_id2], condition, usecond, condparam)
    enddo
    enddo
    ...
    do ax_id3 = ax_id2 + 1, 4
      ...
      call help3d(data_array, [ax_id1, ax_id2, ax_id3], condition, usecond, condparam)
    enddo
    enddo
    enddo
    ...
    histsize(4) = (datasize(4) - mod(datasize(4), nw(4))) / nw(4)
       if (width_id1 /= 1 .and. width_id2 /= 1 .and. width_id3 /=1 .and. width_id4 /=1) then
          ...
          if (usecond) then 
              ...
              call stat1(pack(hist_array, hist_cond >= maxval(hist_cond)*condparam), nw)
          else
  ...
  subroutine help0d(data_array, condition, usecond, condparam)
    ...
    if (usecond) then
      ...
      call stat1(pack(data_array, condition >= maxval(condition)*condparam), nw)
    else
      !call stat(hist_array, nw)
      call stat(data_array, nw)
    end if
  end subroutine help0d
  subroutine help1d(data_array, ax, condition, usecond, condparam)
    ...
    do width_id = 2, maxw(ax)
      ...
      if (usecond) then
        ...
        call stat1(pack(hist_array, histcond >= maxval(histcond)*condparam), nw)
      else
        call stat(hist_array, nw)
      end if
    enddo
  ...
  subroutine help2d(data_array, ax, condition, usecond, condparam)
    integer, intent(in) :: ax(:)
    double precision, intent(in) :: data_array(datasize(1), datasize(2), datasize(3), datasize(4))
    integer, intent(in) :: condition(datasize(1), datasize(2), datasize(3), datasize(4))
    logical(1), intent(in) :: usecond
  ...
  subroutine help3d(data_array, ax, condition, usecond, condparam)
    integer, intent(in) :: ax(:)
    double precision, intent(in) :: data_array(datasize(1), datasize(2), datasize(3), datasize(4))
    integer, intent(in) :: condition(datasize(1), datasize(2), datasize(3), datasize(4))
    logical(1), intent(in) :: usecond
