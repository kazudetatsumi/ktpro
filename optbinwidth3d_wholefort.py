...
def calc_cost3d_f90(A, maxw, condition):
    ...
    lib.cost2d.argtypes = [ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), 
    ...
    result = lib.cost2d(ctypes.byref(ctypes.c_int(maxw[0])), ctypes.byref(ctypes.c_int(maxw[1])), ctypes.byref(ctypes.c_int(maxw[1])), 
...
def run_simu3d():
    ...
    Cn, kaves, deltas = calc_cost3d_f90(A, data, maxw, condition, fflag)
    ...
    opt_indx = (opt_indx[0] + 1, opt_indx[1] + 1, opt_indx[2] + 1 )
    print "opt_indx for Cm with m=",m, ":", opt_indx
