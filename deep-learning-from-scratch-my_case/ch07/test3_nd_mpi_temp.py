...
from conv2_net_nd import conv2Net
...
def run():
    ...
    for train_indx, validation_indx in kf.split(_x_train):
        ...
        if kidx == rank+1:
            ...
            nt = conv2Net(input_dim=(1, 1, _x_train_shape[1]),
                          conv_params={'filter_num':TEMPFN, 'filter_height':1,
                                       'filter_width':TEMPFW, 'pad':0, 'stride':1},
                          hsize=TEMPH, osize=4, wstd=0.01)
    ...
    if rank == 0:
        ...
        print("average acc in validation:{} +- {}".format(ave_acc_validation,
                                                          std_acc_validation))
    
        pb = open('dump3_nd_TEMPFN_TEMPFW_TEMPH.pkl', 'wb')
        #pickle.dump(nt, pb)
