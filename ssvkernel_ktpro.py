import numpy as np
# modified by Kazuyoshi TATSUMI so as to directly use a histogram as an input
# data. 2022/03/23

#def ssvkernel(x, tin=None, M=320, nbs=1e2, WinFunc='Boxcar'):
def ssvkernel(spectra, energy, M=80, winparam=5, nbs=1e2, WinFunc='Gauss'):
    """
    ...
    #W = logexp(np.linspace(ilogexp(0.5 * dt), ilogexp(T), M))
