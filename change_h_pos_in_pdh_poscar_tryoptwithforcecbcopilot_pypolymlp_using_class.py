...
from change_h_pos_in_poscar_class import change_hpos as ch  # noqa: E402


@dataclass
class LogCache:
    f_meV: float = None
    g_meV_per_frac: float = None
...
def make_callback(log: LogCache):
    ...
    def cb(xk):
        ...
        if g_meV_per_frac is None:
            print(
                f"[cycle {k:3d}] f = {f_meV:.6f} meV, "
                "grad = (not computed yet)"
            )
            return
        g_eV_per_Angs = g_meV_per_frac / 1000.0 / conv
        print(
            f"[cycle {k:3d}] f = {f_meV:.6f} meV, "
            f"grad = {g_meV_per_frac:.6e} meV/frac, "
            f"{g_eV_per_Angs:.6e} eV/Å"
        )
...
def get_E0(positions, prj, case):
    need_heavy = (
        np.abs(positions - prj.positions[15, 0]) > 1e-15
        or "E_comp" not in dir(prj)
    )
    if need_heavy:
        print(
            "**get_E0: processes from potential to GetEigen are to be done",
            positions,
            prj.positions[15, 0],
        )
        prj.positions[15, 0] = positions[0]
        prj.potential = getene_wstr(prj.hpos, positions, case).reshape(
            (prj.nx, prj.nx, prj.nx)
        )
        prj.GetVG()
        prj.GetH()
...
def get_grad(positions, prj, case):
    ...
    if need_heavy:
        print(
            "**get_grad: processeses from potential to GetEigen are to be done",
            positions,
            prj.positions[15, 0],
        )
        prj.positions[15, 0] = positions[0]
    ...
    f_meV_per_frac = (
        -np.sum(np.array(fx).reshape(prj.densities[0].shape) * prj.densities[0])
        / np.sum(prj.densities[0])
        * 7.8968828318389797
        * 1000.0
    )
