...
class get_qlist:
    ...
    def get_data(self):
        EC = Cmm.GetHistogramMon(
                runNo=6208, useEiConv=True, LambdaParam="6.321/4.15",
        ...
        self.DAT = Cmm.GetHistogramHW(
                runNo=6208, HwParam="0.00025/-0.05/0.15",
    ...
    def get_sdata(self):
        ...
        DAT = Cmm.GetHistogramHW(
                #runNo=6208, HwParam="0.000025/-0.05/0.15",
                runNo=6208, HwParam="0.0001/-0.05/0.15",
                LambdaParam="6.321/4.15", t0_offset=12325.0,
                useT0ModCorr=False, TimeParam="-1.0/-1.0", UseFastChopper=True,
                tofOffsetFile="none", isHistogram=False)
    ...
    def test_manyo(self, binw):
        ...
        #Cmm.MutiplyConstant(dat=EC, factor=1e-09)
        self.DAT = Cmm.GetHistogramHW(
                runNo=6208, HwParam=binw+"/-0.05/0.15",
                LambdaParam="6.321/4.15", t0_offset=12325.0,
                useT0ModCorr=False, TimeParam="-1.0/-1.0", UseFastChopper=True,
                tofOffsetFile="none", isHistogram=False)
        #Cmm.DoMask(dat=DAT, filename="maskTY.txt")
    ...
    def test_manyo_for_resampled_data(self, intensityb):
        ...
        for ecaidx in range(0, self.DATB.PutSize()):
            for ecidx in range(0, self.DATB(0).PutSize()):
                vecy = ctp.ListToDoubleVector(np.squeeze(intensityb[ecaidx,
                                              ecidx, :]).tolist())
                vece = ctp.ListToDoubleVector(np.squeeze(intensityb[ecaidx,
                                              ecidx, :]**0.5).tolist())
        ...
        ECM = Cmm.ILambdaCorrDNA(dat=self.DATB, ec=EC, useMonEff=True)
        ECM2 = Cmm.SolidAngleCorrDNA(
                dat=ECM, useDetEff=True, useAbsoCorr=False, useEffCorr=False,
                sampletype="sample", sampleDataPath="test_sample_data.dat",
                DetEffDataPath="none")
    ...
    def get_all_sdatabqe(self):
        ...
        self.datasetbqe = {}
        self.datasetbqe['omega'] = omega
        self.datasetbqe['q'] = q
        self.datasetbqe['intensity'] = intensity
    ...
    def spectrabqe(self, qmin, qmax):
        x = np.ravel(self.datasetbqe['q'])
        y = np.ravel(self.datasetbqe['omega'])
        z = np.ravel(self.datasetbqe['intensity'])
        _x = x
        area = np.where((_x <= qmax) & (_x >= qmin))
        x = x[area[0]]
        y = y[area[0]]
...
def samplerun_after_dna():
    save_file = "./srlz/run6208.srlz"
    pklfile = "./srlz/run6208.pkl"
    spectrafile = "./srlz/run6208spectra.pkl"
    qmin = 0.55
    qmax = 0.70
...
def samplerun_after_sdna():
    save_file = "./srlz/run6208s.srlz"
    pklfile = "./srlz/run6208s.pkl"
    spectrafile = "./srlz/run6208sspectra.pkl"
    qmin = 0.55
    qmax = 0.70
    prj = get_qlist(save_file, pklfile=pklfile)
    prj.read_pkl()
...
def samplerun_on_dna_sdata():
    save_file = "./srlz/0000025/run6208s.srlz"
    pklfile = "./srlz/0000025/run6208s.pkl"
...
def manyo_test():
    prj = get_qlist()
    prj.test_manyo("0.000025")
    prj.get_all_databqe()
    print(datetime.datetime.now(), 'org data obtained')
    intensity = prj.dataset['intensity']
    intensity1d = intensity.flatten().astype(int)
    nonzeroidx = np.nonzero(intensity1d)[0]
    # x = np.array([idx for idx in range(0, intensity1d.shape[0]) for
    #              num_repeat in
    #              range(0, intensity1d[idx])
    #              ], dtype=int)
    x = np.array([idx for idx in nonzeroidx for num_repeat in
                 range(intensity1d[idx])], dtype=int)
    nbs = 1
    N = x.shape[0]
    intensityb = np.zeros((intensity.shape[0], intensity.shape[1],
                          intensity.shape[2], nbs))
    for inb in range(nbs):
        print(inb)
        Nb = np.random.poisson(lam=N)
        idx = np.random.randint(0, N, Nb)
        xb = x[idx]
        test_idxs = np.unravel_index(xb, intensity.shape)
        for _idx0, _idx1, _idx2 in zip(test_idxs[0], test_idxs[1],
                                       test_idxs[2]):
            intensityb[_idx0, _idx1, _idx2, inb] += 1
        print(datetime.datetime.now(), '1 resampling ended')
    prj.test_manyo_for_resampled_data(intensityb)
    print(datetime.datetime.now(), '1 resampled manyo-data obtained')
    prj.get_all_sdatabqe()
    print(datetime.datetime.now(), '1 corrected resampled manyo-data obtained')
    prj.spectrabqe(0.55, 0.70)
    print(datetime.datetime.now(), '1 final data obtained')
    #prj.qemap(np.min(prj.dataset['q']), np.max(prj.dataset['q']))
...
def manyo_testtest():
    ...
    prj.get_all_sdata()
    #prj.qemap(np.min(prj.dataset['q']), np.max(prj.dataset['q']))
    prj.spectra(0.55, 0.70)

def check():
    save_file = "./srlz/run6208_half.srlz"
    pklfile = "./srlz/run6208_half.pkl"
    spectrafile = "./srlz/run6208spectra.pkl"
    qmin = 0.55
    qmax = 0.70
    prj = get_qlist(save_file, pklfile=pklfile)
