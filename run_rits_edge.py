...
from edge_fit_kt import get_sim_spectrum
sys.path.append("/home/kazu/denoise")
import bi2d
np.random.seed(129)
#'a0', 'b0', 'a_hkl', 'b_hkl', 'd_hkl', 'sigma0'
...
def get_params(rpkl=None, wpkl=None):
    if rpkl:
        ...
        param_sets = {}
        param_sets['param_name'] = ["a0"]
        param_sets['string'] = ['A0']
        param_sets['lims_mean'] = [[0.2, 0.6]]
        param_sets['lims_scale'] = [[0.3, 1.0]]
        param_sets['lims_xlim'] = [[1., 10.]]
        param_sets['param_name'].append("b0")
        param_sets['string'].append("B0")
        param_sets['lims_mean'].append([0.1, 0.6])
        param_sets['lims_scale'].append([0.1, 1.0])
        param_sets['lims_xlim'].append([30., 40.])
        param_sets['param_name'].append("a_hkl")
        param_sets['string'].append("A_HKL")
        param_sets['lims_mean'].append([0.3, 2.5])
        param_sets['lims_scale'].append([0.1, 1.0])
        param_sets['lims_xlim'].append([40., 50.])
        param_sets['param_name'].append("b_hkl")
        param_sets['string'].append("B_HKL")
        param_sets['lims_mean'].append([0.1, 1.2])
        param_sets['lims_scale'].append([0.1, 1.0])
        param_sets['lims_xlim'].append([15., 40.])
        param_sets['param_name'].append("d_hkl")
        param_sets['string'].append("D_HKL")
        param_sets['lims_mean'].append([2.01, 2.05])
        param_sets['lims_scale'].append([1.5, 2.6])
        param_sets['lims_xlim'].append([4., 20.])
        param_sets['param_name'].append("sigma0")
        param_sets['string'].append("SIGMA0")
    ...
    for pidx in range(len(param_sets['param_name'])):
        ...
        if dim == 1:
            ...
            _params = draw_sample2d_mpi(rg, mean=mean, numsample=1,
                                    scale=scale, xlim=xlim, xsize=32,
                                    ysize=100, rsize=12)
        #if pidx==0:
...
def cycles_mpi(ns=10, dim=2, pklfile='param_sets_sets_bccrev2_2d.pkl'):
    ...
    _param_sets_sets = []
    param_sets = get_params(wpkl='param_sets_bccrev2_edge.pkl')
...
cycles_mpi(ns=1, dim=2, pklfile='param_sets_sets_bccrev2_2d_edge.pkl')
#divide_paramdata()
