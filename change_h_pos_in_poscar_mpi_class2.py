...
class change_hpos():
    def __init__(self, infile, std, edgelength, nx, enefile=None,
        ...
        ''' dx is set as follows since 2022/10/05
            note that this is formally the same for all of the fractional
            /cartecian and scalar/list self.edgelength. '''
            ...
            for i in range(-self.nx//2, self.nx//2+1):
                for j in range(-self.nx//2, self.nx//2+1):
                    for k in range(-self.nx//2, self.nx//2+1):
                        # if (i/self.edgelengthina0[0])**2 +\
                        #        (j/self.edgelengthina0[1])**2 +\
                        #         (k/self.edgelengthina0[2])**2 <\
                        #         self.rg**2:
    ...
    def Plotter(self, label=None):
        ...
        E = self.dataset['E']
        sqw = self.dataset['integratedsqw']
    ...
    def Integrateoverallsolidangle2(self, qlength, Iswrite=True):
        ...
        for it, theta in enumerate(thetas):
            ...
            q = qlength * np.array([np.cos(theta), np.sin(theta)*np.cos(phi),
                                    np.sin(theta)*np.sin(phi)])
            self.GetTransitionMatrix(q, Isplot=False, Iswrite=False)
            if it == 0:
