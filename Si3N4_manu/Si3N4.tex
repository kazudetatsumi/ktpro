...
\begin{figure}[ht]
 \begin{center}
  ...
  diagrams\cite{inttableA} for {\it P$\rm{31}$c} ($\alpha$-Si$_3$N$_4$) and {\it
  P$\rm{6_3}$/m} ($\beta$-Si$_3$N$_4$)
...
$\beta$-Si$_3$N$_4$ to be 170 and 450 W m$^{-1}$ K$^{-1}$, respectively.  The
ratio $\kappa_{zz}/\kappa_{xx}$ in $\beta$-Si$_3$N$_4$ agreed well with the
experimental ratio; $\kappa_{xx}$ and $\kappa_{zz}$ were overestimated by more
than two times those of the corresponding experimental $\boldsymbol{\kappa}$
values. 
...
densities of states (DOS).\cite{phono3py} On the other hand, the previous MD
results indicated that the different stacking manners between the $\alpha$ and
$\beta$ phases altered $\boldsymbol{\kappa}$ significantly.\cite{hirosaki-md} This has not been
...
for a finite temperature of 300 K $\boldsymbol{\kappa}$ calculated with the
...
conductivity, $\boldsymbol{\kappa}^c(\omega)$, with the isotope effect. 
...
cross-section in Fig.~\ref{fig:Fig4_ver5_338}~(b), respectively.  
The profiles at the edges of the cross-sections are present as the corresponding dispersion curves in 
the band diagrams. For clarification purposes, the dispersion curves
corresponding to such profiles for $p$=4 are highlighted by gray color:
It denote the frequency upper bounds among the profiles for $p$=1,..,4. 
...
The most part of the cross-sections for $p$=1, 2, and 3 shows $\omega_\lambda$ of the acoustic phonon
modes. 
...
cell, as in Ref.~\onlinecite{sio2}. The averages are very similar for the $\alpha$ and $\beta$ phases. With
