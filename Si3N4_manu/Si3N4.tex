...
The experimental $\kappa$ of the $\beta$ phase ceramic bulk~\cite{hirosaki} as
plotted with solid circles in Fig.~\ref{fig:Fig1_338}-b fall well between the
theoretical $\kappa$$_{xx}$ and  $\kappa$$_{zz}$, being nearly
inversely proportional to $T$. Simple directional averages of the theoretical
$\kappa_{ii}$ slightly underestimate these experimental bulk thermal
conductivities.  This fact is understood from the experimentally tailored
microstructure containing large $\beta$-Si$_3$N$_4$ grains selectively grown
along the $c$ axis.~\cite{hirosaki} The theoretical bulk thermal conductivities
were fit well with $w=\textcolor{blue}{0.44}$  to the experimental bulk thermal
conductivities. For the effects of lattice defects most of which were grain
boundaries, we included $\tau_{\lambda,\text{bs}}$ with $L =
\textcolor{blue}{0.6}$ $\mu\text{m}$ to further fit the theoretical curve
($w=\textcolor{blue}{0.33}$) to the experimental data.  The $L$ value is
\textcolor{blue}{slightly smaller than} the average grain size (2
$\mu\text{m}$)~\cite{hirosaki} of the experimental polycrystalline sample.
The experimental $\kappa$$_{xx}$ and $\kappa$$_{zz}$ for room temperature are
also plotted by filled squares, which are in-between the theoretical tensor
elements with and without $\tau_{\lambda,\text{bs}}$. 
...
The calculated $\omega_{\lambda}$ are much more increased in the $\beta$ phase
on the acoustic branches from $\Gamma$ to A than to K or M.  This is different
from the case of the $\alpha$ phase; those are increased from $\Gamma$ to A, K,
and M similarly.  This difference in the increments of $\omega_{\lambda}$
between the two phases  is because the $\Gamma$--A path length of the $\beta$
phase is approximately twice that of $\alpha$; the lattice constant $c$ of
$\beta$ is nearly half that of $\alpha$, owing to the difference in the
stacking manner of the basal layers. Normally, optical branches are flat;
however, the upper branches along the $\Gamma$--A path also have significantly
large ${v}_{\lambda,z}$.  These findings on the phonon branches indicate that
the $\beta$ phase contains a larger number of phonon modes whose group velocity
is large and oriented along the $c^*$ axis. We will show this tendency in the next
section.

In Fig.~\ref{fig:Fig4_ver5_338}-c for the $\gamma$ phase, the acoustic phonon
branches show significant linear dispersions on the $\Gamma$--L and $\Gamma$--X
paths.  The roughly constant gradients of $\omega_{\lambda}$ are large,
reflecting the large elastic constants of the $\gamma$ phase as shown in Table
\ref{table:LTC-exp}.
...
In another point of view on $\mathbf{v}_{\lambda}$, Fig.~\ref{fig:Fig3_338}
shows the $\omega_{\lambda}$ distribution on the $b^*$-$c^*$ plane within the
Brillouin-zones of the $\alpha$ and $\beta$ phases.  The distributions on the
other planes containing the $c^*$ axis did not differ much.  Thus, we selected
the $b^*$-$c^*$ plane as a representative one. We show the $\omega_{\lambda}$
distributions for four modes from the lowest frequency because they contribute
significantly to $\boldsymbol{\kappa}$.  Fig.~\ref{fig:Fig3_338}-a for the
$\alpha$ phase shows that the distributions of the $\omega_{\lambda}$ are
fairly isotropic. Their gradients, the group velocities projected onto the
$b^*$-$c^*$ plane, are thus isotropic.  On the other hand, in
Fig.~\ref{fig:Fig3_338}-b for the $\beta$ phase, the iso-frequency lines in
...
In order to rigorously evaluate the contribution of the microscopic phonon
properties to $\boldsymbol{\kappa}$, we examine the microscopic phonon
properties taken over the Brillouin zone. In
Fig.~\ref{fig:Fig5_338_rev} are shown phonon densities of states (DOS),
cumulative thermal conductivities and their frequency derivatives, DOS weighted
with the squares of the group velocity components, $v_{\lambda,x}^2$ and
$v_{\lambda,z}^2$, and distributions of the phonon linewidths. 
