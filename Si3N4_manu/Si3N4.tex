...
than two times \uline{those} 
["that" was written in the corrected Si3N4.tex, instead. Because I compare
$\kappa_{xx}$ and $\kappa_{zz}$ after the semicolon, "those" seems adequate than
"that".]
of the experimental $\boldsymbol{\kappa}$. 
...
systematic understanding. After the methodology \uline{is described}, we
examine the validity of the present results, \uline{through comparison
of} the calculated thermal properties with the available experimental and
theoretical references. 
[I intend that the validity of the present calculation is examined during the
comparison between the present calculation results and the references. But the
original description was misunderstood as
"After the validation, the present calculation results are
...
solution was to be minor for our discussion. Therefore, this research was
limited to use the single-mode RTA to take advantage of \uline{its closed form of
$\boldsymbol{\kappa}$, which can be intuitively understood in
terms of mode-specific phonon properties.}
[If the 'intuitively closed form' can be used to
express that the closed form is intuitively understood, then it is OK. But I can
not judge this and add some words to express the situation correctly.]
...
thermal expansion. \uline{The differences in $\boldsymbol{\kappa}$ for $T$=300, 600,
900, 1200, and, 1500 K, for the $\beta$ phase, were within 1 \%. They 
were similar to those for Si and Ge calculated by Ward {\it{et
al.}}\cite{ward-ltc}.}
[I meant that the degree of the differences in kappa for 300, 600, 900, 1200,
and , 1500 K calculated in the present study is similar to that reported on Si
and Ge by Ward et al.. I modify the representation for correct expression.]
...
\uline{Comparison with the experimental coefficient is useful to validate the present
...
\uline{along the inequivalent reciprocal axes}, with particular focus
on their anisotropy in the $\alpha$ and $\beta$ phases. This was not
investigated in the previous works. \uline{The band diagrams on the
other high-symmetry paths} are almost identical to those reported
earlier\cite{kuwabara,xu} and thus are not shown.
[I mean that selected parts out of the entire band
diagrams are shown here and the other parts are not shown.]
...
\begin{figure}[ht]
 \begin{center}
  ...
  \uline{}[The figure number "FIG.X" is put in series during the tex
  compilation. I do not
  know the tex syntax to express the number for supplemental figure and thus do
  not
  change the form as in the original Si3N4.tex, although your suggestion is
