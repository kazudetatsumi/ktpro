...
\textcolor{red}{The reported values~\cite{zhou,hirao-rev,watari,hirosaki,hirai} of the thermal conductivity of the Si$_3$N$_4$ polymophs were measured on pollycrystalline bulk samples. These values were significantly affected by the contained
	lattice defects, impurities, shapes and orientations of the constituent crystal grains; the thermal conductivity intrinsic to a defect-free single crystal has not been established.~\cite{hirosaki-md} 
To access to the intrinsic thermal conductivity,}
Li {\it et al.}~\cite{li} applied high-resolution thermoreflectance
microscopy to a single $\beta$-Si$_3$N$_4$ grain in a ceramic sample. The analyzed thermal connectivity 
...
obtained by Li {\it et al.}~\cite{li}: The $\kappa_{ii}$ values overestimated the experimental by more than two times. 

The MD results presented that the different stacking orders between $\alpha$-
and $\beta$-Si$_3$N$_4$ altered their LTC largely.  On the other hand, LTCs of
many polymorphs of the wurtzite and zincblende structures were  recently
calculated based on first principles calculation and Boltzman transport theory.
These structures have different stacking orders of the densest atom planes as
ABAB.. and ABCABC...  The different stacking orders in this case merely altered
the LTCs.~\cite{phono3py} \textcolor{blue}{Similar behaviors were also
calculated for the phonon linewidth distribution and phonon density of
states.~\cite{phono3py} In Si$_3$N$_4$, correlation between the LTCs and phonon
properties has not been clarified.  It is interesting to investigate it using
the same approach as in Ref.\onlinecite{phono3py} for $\alpha$- and
$\beta$-Si$_3$N$_4$.}
...
The present study aims to qualitatively understand the LTC tensors between the
three Si$_3$N$_4$ phases by means of the first principles approach.
\textcolor{blue}{We investigate the LTCs of  the $\gamma$ phase as well, for
systematic understanding of the LTCs in the Si$_3$N$_4$ system.  After the
methodology section, we examine the validity of the present LTC results
first.  Our calculated thermal properties are compared with the available
experimental data and theoretical results.  Then we investigate the the
present LTCs in detail on the basis of phonon band structures and linewidths.}
...
volume calculated within the local density approximation
(LDA)~\cite{lda} for the exchange correlation potential was, for
$\beta$-Si$_3$N$_4$, 3 \% smaller than within GGA, which is a typical
volume contraction of LDA. The LTC calculated within LDA was larger by 2.6 \% than within GGA. For
...
IFCs.~\cite{wei-supercell} The supercells were $1\times 1\times2$, $1\times
1\times3$, and $1\times 1\times1$ supercells of the conventional unit cells for
the calculation of the third-order IFCs in $\alpha$, $\beta$, and
$\gamma$-Si$_3$N$_4$, respectively; they were $3\times 3\times4$, $3\times
3\times8$ and $2\times 2\times2$ for the second-order IFCs.  The length of the
induced atomic displacement was set to 0.03 \AA.  Table \ref{table:LTC} shows
the calculated LTC values for several different sets of the supercells,
indicating that our calculated LTCs are reasonably converging with respect to
...
\textcolor {blue}{We examined the effect of thermal expansion on the LTCs
by calculating them with the crystal structures optimized for several finite
temperatures within the quasi-harmonic approximation (QHA)~\cite{dove-p76}.
Each of the crystal structure was obtained for one of the finite temperatures
and 0 GPa by optimizing the lattice volume so as to show a minimum in the
Helmholtz free energy. The LTC of the optimized structure was calculated for
the finite temperature.  This LTC was differed from the LTC for the finite
temperature, calculated using the crystal structure optimized for 0 K and 0
GPa. We consider that this difference is caused by the thermal expansion of the
lattice volume. For $\beta$-Si$_3$N$_4$, across the finite temperatures of  300, 600, 900, 1200, and
1500 K, the relative differences between the LTCs of the two structures were
found less than 1 \%, similar to the case of Si and Ge~\cite{ward-ltc}. For the
present study, these differences are negligible and we adopt the LTCs of the
crystal structure optimized for 0 K and 0 GPa.}
...
present LTC calculation, because the thermal expansion is originated from the
anharmonicity of the interatomic potential as well as LTC. The calculated
values are 4.31$\times 10^{-6}$ and 4.19$\times 10^{-6}$ K$^{-1}$ at 300 K for
the $\alpha$ and $\beta$ phases, while the experimental values were 3.75 and
3.55 ($10^{-6}$ K$^{-1}$)~\cite{minikayev-alpha}. The present calculation
reproduced the experimental tendency where the $\alpha$ phase has a slightly
larger thermal expansion coefficient than the $\beta$ phase, supporting that
the present calculations enable us to qualitatively compare the LTC values of
the Si$_3$N$_4$ phases.
...
\textcolor {red} {Therefore, we discuss the qualitative character of the LTC
using the single-mode RTA solution.}
...
Table \ref{table:LTC-exp} shows the present theoretical LTC values at 300 K.
The LTCs of $\beta$-Si$_3$N$_4$ are markedly more anisotropic than those of
$\alpha$-Si$_3$N$_4$.  The directional averages of our LTCs are 79, 115,  and
...
references. The LTCs of the Slack model are calculated by Morelli {\it et al.}~\cite{morelli} and these values 
are also shown as $\kappa$ in Table \ref{table:LTC-exp}.
Among the comparisons of the theoretical results to the experimental, our $\kappa_{ii}$ agrees better with the experimental, than that of the
MD~\cite{hirosaki-md}. Our directional average (115 Wm$^{-1}$K$^{-1}$) agrees
better with the experimental directional average of 106 Wm$^{-1}$K$^{-1}$, than
the result of the Slack model.

Fig.~\ref{fig:Fig1_338} shows the theoretical LTCs of the $\alpha$  and $\beta$
phases as a function of $T$, together with the reference experimental
data~\cite{hirosaki,hirai,li}. The thermal conductivities over a series of
temperatures were only reported on the polycrystalline bulk samples.  These
bulk thermal conductivities cannot be directly compare with these theoretical
LTC tensors because the microstructures of the bulk samples largely affect the
thermal conductivities:  \textcolor {blue} { The thermal conductivity can be
deviated from $\sum_i \kappa_{ii}/3$, a simple directional average of
$\kappa_{ii},$ because inequivalent weights can be supposed on the
$\kappa_{ii}$ terms  according to the shapes of the constituent crystal
grains.  We represented these inequivalent weights by a parameter
$0\le{w}\le{1}$ and approximated theoretical LTC of the polycrystalline
bulk sample as $\kappa = w\kappa_{xx} + (1-w) \kappa_{zz}$. The parameter
was adjusted to result in the least square difference between the
theoretical and experimental LTC values.  Moreover, the thermal
conductivity can be decreased due to the phonon scattering at lattice
defects in the polycrystalline sample.  We consider this by the simple boundary
scattering model. In this case both the parameters $L$ and $w$ are optimized
