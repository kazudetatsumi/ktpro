...
cross-section, in the band diagram, a thick gray line is used to denote the dispersion curves
corresponding to the profiles at these edges for $p$=4. 
...
specified by $\sum_{i,j}[\frac{e_j(i,\lambda)q_j}{|\mathbf q|}]^2$, where
$e_j(i,\lambda)$ is the $i$-th atom's $j$-th cartesian coordination of
eigenvector and $q_j$ is the $j$-th cartesian coordination of $\mathbf{q}$.
Since  $\sum_{i,j}e_j(i,\lambda)^2=1$, the color contrast indicates the fraction
of the eigenvector component along $\mathbf q$: The brighter (darker) color
...
To check the population ratio between the phonon modes in the two branches, in
Fig.~\ref{fig:Fig7_338}(b) for the $\beta$ phase, we insert a straight line
between the two branches and count the numbers of the phonon modes above and
below the line. They are 145 and 67, whose ratio is reasonable because the large
contrast among the plots in Fig.~\ref{fig:Fig7_338}(b) for the $\beta$ phase
indicates that the atomic vibration for the most of the phonon modes is 
approximately parallel or perpendicular to the $ab$ plane.
