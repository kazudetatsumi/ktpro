...
\kappa_{ii}/3$ agrees better with the experimental
average, than the Slack model does.
...
counter maps for the four lowest-frequency bands, because these bands
...
representative.  In the $\alpha$ phase, the $\omega_{\lambda}$ distributions are nearly
...
over the Brillouin zone because $\boldsymbol{\kappa}$ is a quantity
integrated there. In order to investigate these properties with respect to the
phonon branches, we plot in Fig.~\ref{fig:Fig5_338_rev} the frequency
distribution of these properties: Phonon densities of states (DOS) in
Fig.~\ref{fig:Fig5_338_rev}-a show distributions of heat carriers. 
Distinct first peaks in the DOS are denoted by arrows. They roughly correspond
to the flattening of the acoustic branches near the Brillouin zone boundaries.
Cumulative thermal conductivities $\boldsymbol{\kappa}^c$ in
...
first derivatives are also shown
in order to show the contributions clearly. We weighted the DOS with
$v_{\lambda,i}^2$ and they are shown in Fig.~\ref{fig:Fig5_338_rev}-c. These
profiles show both of the impacts of $v_{\lambda,i}$ and number of heat
...
Among the panels in Fig.~\ref{fig:Fig5_338_rev}, the $\gamma$ phase has DOS, WDOS and
$\Gamma_\lambda$ distribution much different from
those of the other phases, consistently with the large
...
the next largest to the WDOS for $v_{\lambda,z}$ of the $\beta$ phase.
...
that of the other phases. This is consistent with the larger linewidths.
Because of the opposite impacts of the linewidths and WDOS, the $\kappa^c_{xx}$
is relatively small. It qualitatively resembles to the $\kappa^c_{xx}$ of the
$\beta$ phase.
...
large $\mathbf{v}_\lambda$. The large $\mathbf{v}_\lambda$ in this frequency
range reflect the characteristic points in the band diagram as investigated in
the previous section; the anisotropic dispersions in the acoustic phonon
branches and the large gradients in the low frequency optical phonon branches.
...
because the formula of $\Gamma_{\lambda}$ contains delta functions corresponding to the
...
scattered roughly on two lines. This difference can be explained by the
vibration directions. In Fig.~\ref{fig:Fig7_338}-a, the $\Gamma_\lambda$ are
...
They are not configured toward high $\kappa_{xx}$.
