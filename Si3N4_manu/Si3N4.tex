...
$\mathbf{q}$-point.  We use $e_\lambda^{i,j}$ to denote the eigenvector
...
the group velocities all over the Brillouin zone, the cross-section of the
phonon frequency distribution is shown in Fig.~\ref{fig:Fig4_ver5_338}~(b). Since the
cross-section on the planes 
containing the $c^*$ axis did not differ much from each other 
...
Fig.~\ref{fig:Fig4_ver5_338}~(a) correspond to the left and bottom
edges of the cross-section in Fig.~\ref{fig:Fig4_ver5_338}~(b), respectively.
In order to show more clearly the relationship between the band diagram and the
cross-section, in the band diagram, a thick gray line is underlaid for the dispersion curves
corresponding to the profiles at these edges of the cross-section for $p$=4. 
...
In the cross-sections of $p$=1, 2, and 3 for the $\beta$ phase, the gradients of
...
plots of $(\tau_{\lambda,\text{ph-ph}},\omega_\lambda)$ in
...
$|\Phi_{\lambda\lambda'\lambda''}|^2$ in the narrower frequency range. 
...
The brighter color indicates the more longitudinal character of the phonon mode.  In
...
phase,  a phonon mode of the vibration parallel to the $ab$ plane is more hindered by
...
and the phonon modes whose atomic vibration is perpendicular to the $ab$ plane.
