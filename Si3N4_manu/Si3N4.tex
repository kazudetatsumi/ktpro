...
$\tau_{\lambda,\text{ph-ph}}=(2\Gamma_\lambda(\omega_\lambda))^{-1}$, by 
\begin{align}
 \label{eq:linewidth}
...
next section. $N_2(\mathbf{q},\omega)$ is a weighted joint DOS (WJDOS)\cite{phono3py},
\begin{align}
 \label{eq:jdos}
...
$N_2$ in Eq.~(\ref{eq:linewidth}) for the analysis given below.
...
$\mathbf{q}$-points. The intensities are scaled with $Z^2$ where $Z$ is the
