...
closely resemble that distribution for the  $\kappa_{xx}$ of $\beta$-Si$_3$N$_4$
...
additional two layer structures of C and D, which are related to the A and B by
...
$\boldsymbol{\kappa}$ of many polymorphs of the zincblende and wurtzite
structures. Their crystal structures have stacking orders of the densest atom
planes as ABAB.. and ABCABC...  The different stacking orders merely altered the
$\boldsymbol{\kappa}$.~\cite{phono3py} The phonon linewidth
distribution and phonon density of states were not altered  as
...
The available experimental thermal conductivities in the Si$_3$N$_4$ system
were measured on the polycrystalline samples and not measured from any
...
600, 900, 1200, and 1500 K, the differences were found within 1 \%, similar
...
values, but kept the $\kappa$$_{ii}$ in the high temperature side severely apart
from the experimental values. At present, the reason for the discrepancy between
...
significant lattice defects existed in the as-deposited sample as pointed out by
Hirosaki {\it et al.}~\cite{hirosaki-md} and the simple phonon boundary
scattering model may fail to describe their effects on the
$\kappa_\mathrm{polycrystal}$. 
...
phase is nearly half that of the $\alpha$ phase, owing to the different
stacking manners of the basal layer structures. The anisotropic dispersions
...
$\boldsymbol{\kappa}$. Here we more comprehensively investigate the
characteristic points in the $\boldsymbol{\kappa}$ by using the phonon
properties existing in the closed form of RTA in Eq.~(\ref{eq:kappa}). These
properties are taken over the Brillouin zone, similar to $\boldsymbol{\kappa}$.
In order to investigate these properties with respect to the phonon modes, we
show in Fig.~\ref{fig:Fig5_338_rev} frequency distributions of these properties:
Phonon densities of states (DOS) in Fig.~\ref{fig:Fig5_338_rev}-a are viewed as
frequency distributions of heat carriers.  In Fig.~\ref{fig:Fig5_338_rev}-a
distinct peaks at the lowest frequencies are denoted by arrows. Considering the
band diagrams, these peaks are approximately associated with flattening of
acoustic phonon branches near Brillouin zone boundaries.  In
Fig.~\ref{fig:Fig5_338_rev}-c, $\boldsymbol{\kappa}^c$ and their first
...
DOS with \rm{v}$_{\lambda,i}^2$. They reveal the impacts of \rm{v}$_{\lambda,i}$
and number of heat carriers. We abbreviate these weighted DOS as WDOS. Phonon
linewidths are a remaining important property.  They are shown as scatter plots
...
over two frequency ranges for $\omega_\lambda$ and all indices in
...
$\rm{v}_\lambda$ attribute to the significant increase in
$\boldsymbol{\kappa}^c$ in the higher frequency side of the DOS peak. The large
$\rm{v}_\lambda$ in this frequency range are consistent with the band diagram we
have examined. In Figs.~\ref{fig:Fig5_338_rev}-b and c, the profiles of
$\frac{d\boldsymbol{\kappa}^c_{ii}}{d\omega_\lambda}$ are qualitatively
explained by the WDOS with $v_{\lambda,i}^2$ of the same directional indices.
...
$\{\lambda, \lambda', \lambda''\}$ involved in the three-phonon scattering (the
phase space available for the three-phonon scattering). The relationship was
demonstrated among several crystals of the diamond and zincblende structures.
Closely related to this, the present linewidth $\Gamma_\lambda$ depends on the
number of configurations for the available two phonons, $\{\lambda',
\lambda''\}$. A distribution of the configurations is represented as a joint
...
energy of many zincblende and wurtzite polymorphs~\cite{phono3py}.  Following to
...
and low-frequency optical phonon branches are relatively small as compared with
the total number of phonon modes. 
Therefore JDOS are mainly determined by the other many phonon modes, even if the
frequency is close to the $\omega_\lambda$ of the former modes.
Table.~\ref{table:aveavepp} indicates that the $\Phi_{\lambda\lambda'\lambda''}$
are similar between the two phases.  With these similar impacts of the JDOS and
$\Phi_{\lambda\lambda'\lambda''}$, $\Gamma_\lambda$ in
