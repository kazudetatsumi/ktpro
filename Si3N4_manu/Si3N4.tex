...
frequency distributions.  The $c_\lambda$ seen within the sum in the rihgt hand
side of the formulae for $\boldsymbol{\kappa}$ in Eq.~(\ref{eq:kappa}) and
$\boldsymbol{\kappa}^c$ in Eq.~(\ref{eq:cum-kappa}) is approximately constant
for 300 K. Thus we disregard it for simplicity. If $\tau_\lambda$ and
$\mathbf{v}_\lambda$ are assumed constant, then the $\omega$-derivative of
Eq.~(\ref{eq:cum-kappa}) is reduced to phonon density of states (DOS) per
volume,  
\begin{align}
 \label{eq:dos}
...
\end{align}
In this context $g(\omega)$ can be viewd as frequency distributions of heat
carrier density.  If $\mathbf{v}_\lambda$ is additionarily considered in the
sum,  
\begin{align}
 \label{eq:wdos}
 ...
 \delta(\omega-\omega_{\lambda}),
\end{align}
which infomrs the impacts of both of the $\mathbf{v}_\lambda$ and heat carrier
density.  $g(\omega)$, $\boldsymbol{h}(\omega)$, and, $\boldsymbol{k}^c(\omega)$
accompanied with its derivative are shown in Figs.~\ref{fig:Fig5_338_rev}-a, b,
and, c, respectively.  As for the remaining term of $\tau_\lambda$, phonon
linewidths are shown as scatter plots of $(\Gamma_\lambda,\omega_\lambda)$ in
Fig.~\ref{fig:Fig5_338_rev}-d.  The effects of the isotope scattering and
boundary scattering are not included in Fig.~\ref{fig:Fig5_338_rev}-c for
simplicity.
...
$\sim$10 THz, respectively, largely contribute to the $\boldsymbol{\kappa}$. We
...
Next, we compare the properties between the $\alpha$ and $\beta$ phases. Because
the linewidths are similar between the two phases, they have similar impacts on
the $d\boldsymbol{\kappa}^c/d\omega$. The diffenet behaviours of the
$d\boldsymbol{\kappa}^c/d\omega$ between the two phases are simply ascribed to
the different $\boldsymbol{h}$. Moreover, because the $g(\omega)$ are also
similar beteen the two phases, the distributions of heat carrier density have
similar impacts on $d\boldsymbol{\kappa}^c/d\omega$. Therefore, between the
phases, the different anisotropy and sterngth of $\mathbf{v}_\lambda$ simply
account for the different behaviors of the $\boldsymbol{\kappa}$. 
...
the crystal structure. The most significant difference for the
$\boldsymbol{\kappa}$ is in the $\Gamma_\lambda$. For $\omega \lesssim 10$, the
linewidths are approximately twice larger than those of the other phases. We
will examine this details later.  As a result of the large $\Gamma_\lambda$ the
$\kappa_{xx}$ is not so large and the $\kappa^c_{xx}$ is similar to that  of the
$\beta$ phase.
