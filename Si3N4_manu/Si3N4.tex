...
can be viewed as frequency distributions of heat carriers.  
In Fig.~\ref{fig:Fig5_338_rev}-b, $\boldsymbol{\kappa}^c$ and their first
derivatives are shown in order to find the phonon mode largely contribution to
the $\boldsymbol{\kappa}$ clearly: In the $\alpha$, $\beta$, and $\gamma$
phases, the phonon modes upto $\sim$6, $\sim$12 and $\sim$10 THz largely
contribute to the $\boldsymbol{\kappa}$, respectively.  We confirm that the
$\omega_\lambda$ shown in Fig.~\ref{fig:Fig3_338} are within these frequency
ranges and the bands significantly contribute to the $\boldsymbol{\kappa}$, as
mentioned in the previous section.  In Fig.~\ref{fig:Fig5_338_rev}-c, we show
weighted DOS with the vector-direct-product of group velocities (WDOS),
$\boldsymbol{h}(\omega)$,
\begin{align}
 \label{eq:wdos}
...
\end{align}
where $Z$ is the number of formula units in the unit cell. WDOS contains the
information of $\mathbf{v}_\lambda$ and number of heat carriers.  Phonon
linewidths are shown as scatter plots of $(\Gamma_\lambda, \omega_\lambda)$ in
Fig.~\ref{fig:Fig5_338_rev}-d.  $c_\lambda$ is approximately $k_\rm{B}$ at 300 K
and disregarded in our discussion. 
...
(1) The first DOS peak is located at the higher frequency ($\sim$9 THz) than
those in the $\alpha$ and $\beta$ phases ($\sim$6 THz).
This is consistent with the band diagram we have examined. At
the lower frequencies than the peak, the intensities are the lowest among
the three phases, indicating the lowest distribution of heat carriers. 
...
(3) In this $\omega$ range,
the $\gamma$ phase has approximately twice larger $\Gamma_\lambda$ than the
other phases. We will examine this details later.
These characters in the $\gamma$ phase are responsible for that 
the $\kappa_{xx}$ is  not so large and the $\kappa^c_{xx}$ is similar to
that  of the $\beta$ phase.

Next, we compare the properties between the $\alpha$ and $\beta$ phases.  The
$h_{ii}$ of the $\beta$ phase with $ii=xx,zz$ are larger than those of the
$\alpha$ phase.  The linewidths and DOS are similar between the two phases and
...
contributing to the $\boldsymbol{\kappa}$, among the two terms of $D_2^{(1)}$
and $D_2^{(2)}$ in Eq.~(\ref{eq:jdos}), dominant is $D_2^{(1)}$.  The
$D_2^{(1)}$ basically
...
to the gap.  Because the gap is originated from the different 
...
the acoustic and low-frequency optical phonon branches, which determine the
characteristic behavior of the $\boldsymbol{\kappa}$, are much fewer than the
other phonon modes.  The JDOS are mainly determined by the other phonon
