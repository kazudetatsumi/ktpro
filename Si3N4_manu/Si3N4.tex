...
axis direction.  The CD layers are related to AB by the $c$ glide
operation,~\cite{hampshire} along which the unit cell periodicity of the
$\alpha$ phase is approximately two times longer than that of the $\beta$
phase, with lattice constants of $c$ = 5.62 \AA~\cite{yashima} and 2.91
\AA~\cite{boulay}, respectively.  
\begin{figure}[ht]
 \begin{center}
...
\textcolor {blue}{Because experimental data are measured at finite
temperatures, we checked the effect of thermal expansion on the calculated
LTC values: We compared the LTC values calcualted with the crystal
structure originally optimized at 0 K and the LTC values calculated with
the crystal structure optimized at finite temperatures within the
quasi-harmonic approximation (QHA).~\cite{dove-p76} For $T$ = 300, 600,
900, 1200 and 1500 K, in  $\beta$-Si$_3$N$_4$, the differences between the
two kinds of LTC values were found less than 1 \%. The degree of the
differences is similar to the case of Si and Ge~\cite{ward-ltc}. For the
present study, this effect is negligible and the LTC values calculated with
the crystal structure optimized 0 K are hereafter shown.}
