...
dispersion-relations.  Their gradients are large, reflecting the large $B$ of
the $\gamma$ phase.
...
Fig.~\ref{fig:Fig3_338} shows contour maps of $\omega_{\lambda}$  on the
...
phase, the $\omega_{\lambda}$ distributions and thus $\mathbf{v}_{\lambda}$ are
nearly isotropic. In the $\beta$ phase, the contours are rather parallel to the
$b^*$ axis and thus the $\mathbf{v}_{\lambda}$ tend to 
orient toward the $c^*$ axis direction. This confirms the large anisotropy
of the $\mathbf{v}_{\lambda}$ in the $\beta$ phase for the acoustic and
low-frequency optical phonon branches.
...
$\boldsymbol{\kappa}$. Here we more completely investigate the
$\boldsymbol{\kappa}$ based on the phonon properties composing the RTA closed
form in Eqs.~(\ref{eq:kappa},\ref{eq:cum-kappa}). 

The mode heat capacity, $c_\lambda$, in
...
in the $\boldsymbol{\kappa}^c$ and their derivative for consistency with
$\Gamma_\lambda$.
...
confirm that the $\omega_\lambda$ shown in Fig.~\ref{fig:Fig3_338} are smaller
than these upper bounds and those bands significantly contribute to the
$\boldsymbol{\kappa}$.  
...
different behaviors of the $d\kappa_{ii}^c/d\omega$.
...
These $N_2^{(1)}$ are similar among the three phases.
...
the frequency ranges for $\omega_\lambda$ so that the lower frequency range
...
to the $\boldsymbol{\kappa}$. The group velocities alone largely depend on the
stacking manners and qualitatively explain the behaviours of
$\boldsymbol{\kappa}$.
...
In contrast to $\alpha$- and $\beta$-Si$_3$N$_4$, in the case of the zincblende and wurtzite
