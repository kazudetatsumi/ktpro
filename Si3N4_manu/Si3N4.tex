...
closely resemble that for $\kappa_{xx}$ of $\beta$-Si$_3$N$_4$
...
constituent crystal grains;~\cite{hirosaki-md} the thermal conductivities
intrinsic to the defect-free single crystals have not been established. As an
experimental approach for them, Li {\it et al.}~\cite{li} applied the
...
respectively
correspond to the $xx$ and $zz$ elements of the lattice thermal
...
{\it et al.}~\cite{hirosaki-md} estimated the $\boldsymbol{\kappa}$ by applying the
...
$\beta$-Si$_3$N$_4$ agreed well with the experimental ratio: The $\kappa_{ii}$
values overestimated the experimental values more than twice. 


Based on a first principles calculation and Boltzmann transport
theory~\cite{phono3py}, Togo {\it{et al.}} recently calculated
$\boldsymbol{\kappa}$ of many polymorphs of the wurtzite- and zincblende-type
...
merely altered the $\boldsymbol{\kappa}$.~\cite{phono3py} The phonon linewidth
distribution and phonon density of states were merely altered as
...
same approach as Togo {\it{et al.}}\cite{phono3py} did.
...
references.  Then we investigate the characteristics in the calculated
...
crystals. In order to consider the effect of various lattice defects in the
polycrystalline samples, such as grain boundaries, impurities, and vacancies, we
crudely took them into account by a relaxation time
...
path. We consider $\tau_{\lambda,\text{bs}}$ as a variable parameter and
included it to some of the $\boldsymbol{\kappa}$ according to Matthiessen's
rule.
...
$\beta$-Si$_3$N$_4$, 3 \% smaller than with GGA, which is a typical volume
contraction of LDA. The $\boldsymbol{\kappa}$ calculated with LDA was larger by
2.6 \% than with GGA. For our discussion, this difference is enough small,
therefore the impact of choice of exchange correlation potential is considered
to be minor in our study.
...
approaches~\cite{wei-supercell}. For this calulation, we adopted the following
...
constants.  The length of the induced atomic displacement was set to 0.03 \AA.
...
differences as the effect of thermal expansion.  For $\beta$-Si$_3$N$_4$ and
temperatures of 300, 600, 900, 1200, and 1500 K, the differences were found
within 1 \%, similar to the case of Si and Ge~\cite{ward-ltc}. For the present
study, these differences are negligible and we adopt the $\boldsymbol{\kappa}$
calculated with the structure which was initially optimized for 0 K.
...
interest. Therefore, we discuss the lattice thermal conductivities using  the
results of the single-mode RTA solution.
...
for the references.  Previously Morelli {\it et al.}~\cite{morelli} employed the
Slack model to estimate the lattice thermal conductivities of the three phases.
They are shown as $\kappa$ in Table \ref{table:LTC-exp}. For the $\beta$ phase,
compared to the $\boldsymbol{\kappa}$ of the molecular
dynamics~\cite{hirosaki-md}, our $\boldsymbol{\kappa}$ agrees better with the
experimental $\boldsymbol{\kappa}$.  Also, compared to the $\kappa$ of the Slack
model, our directional average $\sum_i \kappa_{ii}/3$ is much closer to the
...
for a series of temperatures were only reported on the polycrystalline bulk
samples and measured by the laser flash method.  These thermal conductivities
(denoted as $\kappa_\mathrm{polycrystal}$) cannot be directly compared with the
calculated intrinsic $\boldsymbol{\kappa}$ because they largely depended on the
...
$\Gamma$ to K or M. In Fig.\ref{fig:Fig4_ver5_338}-a of the $\alpha$ phase, the
corresponding $\omega_\lambda$ increase similarly among the paths. This is due
to the different $\Gamma$-A path lengths.  The $\beta$ phase has an
...
gradients of the band dispersions of the low frequency optical phonon branches.
This indicates that the phonon modes on these branches have large
\rm{v}$_{\lambda}$ as well.
...
densities of states (DOS), $g(\omega)$, in Fig.~\ref{fig:Fig5_338_rev}-a are
viewed as frequency distributions of heat carriers.  In DOS, a low-frequency
main peak is denoted by an arrow. Considering the band diagrams, this peak is
approximately associated with flattening of acoustic phonon branches near
Brillouin zone boundaries.  In Fig.~\ref{fig:Fig5_338_rev}-b,
...
vector-direct-product of group velocities (WDOS),
\begin{align}
 \label{eq:wdos}
...
are a remaining important property.  They are shown as scatter plots
...
acoustic phonon branches with the large gradients.  Reflecting this feature, in
the lower frequency side of the peak, the DOS has the lowest intensities among
the three phases. This means the smallest distribution of heat carriers. 

(2) At most of the frequencies with phonon modes largely contributing to
$\boldsymbol{\kappa}$, the WDOS is the next largest to $h_{zz}$ of the $\beta$
phase. This means the large $\rm{v}_\lambda$ of the phonon modes in the acoustic
phonon branches.

(3) At the same frequencies, the linewidths are the largest among the three
phases. For this point, we investigate the $|\Phi_{\lambda\lambda'\lambda''}|^2$
included in Eq.~(\ref{eq:linewidth}).  In Table.~\ref{table:aveavepp}, their
averages are taken over frequency ranges of 0--15 and 0--30 THz for
$\omega_\lambda$ and all indices in $\lambda'$ and $\lambda''$. The average over
$\omega_\lambda$ in 0--15 THz is much larger than that of the other phases. The
large $|\Phi_{\lambda\lambda'\lambda''}|^2$ likely explain the largest
linewidths. This will be futher mentioned later.

In spite of the large  $\rm{v}_\lambda$ of the phonon modes in the acoustic
phonon branches, owing to the large linewidths and small DOS, the
$\kappa^c_{xx}$ is not so large and resembles the $\kappa^c_{xx}$ of the $\beta$
phase.
...
We turn to compare the properties between the $\alpha$ and $\beta$ phases.  It
...
range, WDOS of the $\beta$ phase show still large intensities. The linewidths as
well as DOS are distributed similarly between the two phases. Thus the large
$\rm{v}_\lambda$ attribute to the significant increase in the
$\boldsymbol{\kappa}^c$ there. The large $\rm{v}_\lambda$ in this frequency
range are consistent with the band diagram we have examined. In
Figs.~\ref{fig:Fig5_338_rev}-b and c, the profiles of
$\frac{d\boldsymbol{\kappa}^c_{ii}}{d\omega_\lambda}$ are qualitatively
explained by the $h_{ii}$ with the same indices.
With the similar  DOS and $\Gamma_{\lambda}$, the different anisotropy in the
$\mathbf{v}_\lambda$ simply accounts for the different anisotropy in
$\boldsymbol{\kappa}$. 
...
scattering~\cite{Lindsay}. This inverse proportionality was observed among
several crystals of the diamond and zincblende structures.  Closely related to
this, the present linewidth $\Gamma_\lambda$ depends on the number of
configurations for the available two phonons, $\{\lambda', \lambda''\}$. It also
depends on $|\Phi_{\lambda\lambda'\lambda''}|^2$. We examine these terms
one-by-one. A distribution of two-phonon configurations is represented as a joint
...
employ the JDOS to intuitively examine the similarity between the
...
the total number of the other phonon modes. 
...
WJDOS were negligible. The terms of the two classes corresponding to 
$D_2^{(1)}$ and $D_2^{(2)}$ in JDOS are denoted as $N_2^{(1)}$ and $N_2^{(2)}$. 
The weighting factors reduce the $N_2^{(2)}$ near 0 THz and enhance the
$N_2^{(1)}$ in the high frequency range. But the $N_2^{(2)}$ are dominant in the
low frequency range where the phonon modes largely contribute to the
$\boldsymbol{\kappa}$. These $N_2^{(2)}$ have similar intensities there,
having insignificant differences for the impacts on the linewidths
between the $\alpha$ and $\beta$ phases. In the $\gamma$ phase, the large
$|\Phi_{\lambda\lambda'\lambda''}|^2$ attribute to the large $\Gamma_\lambda$. 

\begin{figure}[ht]
 \centering
  \includegraphics[width=\linewidth]{figure_wjdos.eps} \caption{(color
	  online) Comparison of WJDOS at $\mathbf{q}=(0,0,0)$ among the three phases. 
		  } \label{fig:Fig_wjdos} 
 \centering
\end{figure}
...
$|\Phi_{\lambda\lambda'\lambda''}|^2$, $\Gamma_\lambda$ in
Fig.~\ref{fig:Fig5_338_rev}-d are similar.  
...
Fig.~\ref{fig:Fig7_338}-b, a straight line roughly assigns the phonon modes to
the two branches. The numbers of the phonon modes assigned to the upper and
lower branches are 157 and 58, whose ratio is consistent to the population ratio
of the vibration modes along and out of the $ab$ plane.
...
modes whose number is relatively small.
