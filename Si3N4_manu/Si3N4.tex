...
temperature dependence of the experimental $\kappa_\mathrm{polycrystal}$ of a
chemically vapor-deposited $\alpha$-Si$_3$N$_4$ sample~\cite{hirai} is deviated
from inverse proportion considerably, intersecting the theoretical $\kappa_{ii}$.  Thus
...
layer structures. Normally, optical branches are flat; however, the $\beta$
phase has significantly large gradients in its low frequency optical phonon
branches. 
These findings indicate that the $\beta$ phase contains a larger number
...
We show the anisotropy in the group velocities on another geometry, that is, on a
cross-section of the Brillouin-zone.  In Fig.~\ref{fig:Fig3_338}, we show
distributions of $\omega_{\lambda}$  on the $b^*c^*$ plane.  The distributions for four bands from
...
the group velocities projected onto the plane, are thus nearly
...
distribution of the heat carriers. 
The distinct first peaks in the DOS are indicated by arrows. They are roughly
related to the flattening of the acoustic branches near the Brillouin zone
boundaries.  The cumulative thermal conductivities $\boldsymbol{\kappa}^c$ show
the frequencies where the phonons largely contribute to the
$\boldsymbol{\kappa}$. Their first derivatives are also shown to clearly
identify these frequencies. By weighting the DOS with $v_{\lambda,i}^2$, we show
the impacts of the $v_{\lambda,i}$ and the carrier density together. We
abbreviate these weighted DOS as WDOS. The phonon linewidth is the remaining
important property. To avoid from double counting of the carrier density,
scatter plots of $(\Gamma_\lambda, \omega_\lambda)$  are employed to compare
their impacts. 

In Fig.~\ref{fig:Fig5_338_rev}, compared the four kinds of phonon properties among the three phases, the DOS,
WDOS and $\Gamma_\lambda$ distribution of the $\gamma$ phase are much different
from those of the other phases. These large differences are consistent with the large
...
consistent with the large bulk modulus. (2) The WDOS is the next largest to the
WDOS with the $v_{\lambda,z}^2$ of the $\beta$ phase in the most frequencies
with large $\frac{d\kappa^c_{ii}}{d\omega_\lambda}$.  This is also explained by
the large gradients of the dispersions in the acoustic phonon branches. (3) The
linewidths are also larger than the other phases in the low frequency range. For
this point, we investigate the three-phonon-scattering strength $\Phi_{\lambda\lambda'\lambda''}$.
In Table.~\ref{table:aveavepp}, the magnitudes of
$\Phi_{-\lambda\lambda'\lambda''}$ are compared as their averages over two
frequency ranges of $\omega_\lambda$ and all indices in $\lambda'$ and $\lambda''$. Its
average over $\omega_\lambda$ in 0--15 THz is much larger than those of the other
phases. This is consistent with the larger linewidths.  Because of the {\it
opposite} impacts of the linewidths and WDOS, the $\kappa^c_{xx}$ qualitatively resembles
to the $\kappa^c_{xx}$ of the $\beta$ phase.
...
We hereafter focus on the comparison between the $\alpha$ and $\beta$ phases.
It is interesting that $\boldsymbol{\kappa}^c$ in the $\beta$ phase
increase significantly in the higher frequency range beyond the first peak in
the DOS. In this
frequency range, the WDOS are significantly different: Those of the $\beta$
phase show still large intensities.
The linewidths are distributed in a similar
way for both of the phases.  The DOS are also similar between them. 
Thus the interesting behaviour of $\boldsymbol{\kappa}^c$ is ascribed to the
large $\mathbf{v}_\lambda$. The large $\mathbf{v}_\lambda$ in this high frequency
range are consistent with the anisotropic dispersions of the acoustic phonon
branches and the large gradients of the low frequency optical phonon branches.
As for the anisotropy in the $\boldsymbol{\kappa}$, the same logic is
applicable: There are close similarities between the
$\frac{d\boldsymbol{\kappa}^c}{d\omega_\lambda}$ and WDOS for the same
directional indices, {\it{i}}. The difference in the $\mathbf{v}_\lambda$
anisotropy simply accounts for the difference in the $\boldsymbol{\kappa}$
anisotropy between the two phases.  

It is still curious that the $\Gamma_\lambda$ are similar between these phases
although their $\mathbf{v}_\lambda$ have marked differences.  Thus we investigate
this further.
As for not $\Gamma_{\lambda}$ but $\boldsymbol{\kappa}$, previously, Lindsay {\it et
al.}~\cite{Lindsay} showed a significant positive correlation between the
...
three-phonon scattering), on many crystals of the zincblende structure. 
...
because the formula of $\Gamma_{\lambda}$ contains the delta functions corresponding to the
selection rule of the three-phonon scattering~\cite{phono3py}. 
The distribution of these configurations is represented
...
vector and otherwise zero. 
The JDOS were used to analyze the behavior of the linewidth
$\Gamma_{\lambda}$ of Ge$_2$Sb$_2$Te$_5$~\cite{mukhopadhyay-ltc} and the imaginary part of the self
energy $\Gamma_{\lambda}(\omega_\lambda)$ of many zincblende and
wultzite polymorphs~\cite{phono3py}.  Following to these
studies, we use the JDOS to examine the similarity in the $\Gamma_{\lambda}$
appeared for both of the $\alpha$ and $\beta$ phases.

Fig.~\ref{fig:Fig6_338} shows the frequency-functions of the JDOS at different
...
region responsible for the $\boldsymbol{\kappa}$, among the two terms of
the $D_2^{(1)}$ and $D_2^{(2)}$ in Eq.~(\ref{eq:jdos}), dominant is the $D_2^{(2)}$
which basically corresponds to the half part ($\omega \geq  0$) of the
...
curves reflect this DOS feature, dropping suddenly around 0 THz and showing a
...
DOS across the gap.  Because the gap is basically originated from the
differences in the vibrations of the planer NSi$_3$ contained in both of the
$\alpha$ and $\beta$ phases~\cite{kuwabara}, the major shapes of the $D_2^{(2)}$,
...
the JDOS and $\Phi_{-\lambda\lambda'\lambda''}$ on the $\Gamma_\lambda$,
...
As a small but interesting difference in the linewidths shown in
Fig.~\ref{fig:Fig5_338_rev}-d, the $\Gamma_\lambda$ below 5 THz are aligned on a
single smooth line in the $\alpha$ phase, while those in the $\beta$ phase are
scattered roughly onto two lines. This difference can be explained by the
vibration directions shown in Fig.~\ref{fig:Fig7_338}. In
...
sums have no clear contrast to distinguish the two branches in the $\beta$
...
that  $\Gamma_\lambda$ are large for the vibrations along the $ab$ plane.
Therefore, within the single-mode RTA, for the phonon modes below 5 THz, all of
which belong to the acoustic phonon branches, the vibration modes along the $ab$
...
largely alter the
$\boldsymbol{\kappa}$. In $\alpha$-Si$_3$N$_4$, the $\boldsymbol{\kappa}$ are rather isotropic, while
the $\kappa$$_{zz}$ of the $\beta$ phase is much larger than the $\kappa$$_{xx}$, showing
remarkable anisotropy in the $\boldsymbol{\kappa}$. 
...
to the $\boldsymbol{\kappa}$. The group velocities are significantly different
between these phases; the linewidths are basically similar due to the similar
impacts of the three-phonon-scattering strengths and JDOS.
Therefore the difference in the group velocities alone qualitatively explains
the difference in the $\boldsymbol{\kappa}$ anisotropy between the $\alpha$ and
$\beta$ phases.

3) In the $\gamma$ phase, the $\kappa^c$ is similar to
$\kappa_{xx}^c$ of $\beta$-Si$_3$N$_4$.
Its DOS, group velocities and linewidths are much different from those of the
other phases, consistently with the large differences in the crystal structures
