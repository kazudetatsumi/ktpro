...
\begin{figure*}[ht]
 \begin{center}
  \includegraphics[width=0.9\linewidth]{figure_dos_jdos_kc_m1010_dos_enlarge_wo_arrows.eps}
...
The properties are taken over the
Brillouin zone, similar to $\boldsymbol{\kappa}$.  In order to investigate the
$\boldsymbol{\kappa}$ with respect to the phonon modes, we show in
Fig.~\ref{fig:Fig5_338_rev} frequency distributions of these properties:
Phonon densities of states (DOS) shown in Fig.~\ref{fig:Fig5_338_rev}-a can be
viewed as frequency distributions of heat carriers.  In
Fig.~\ref{fig:Fig5_338_rev}-b, $\boldsymbol{\kappa}^c$ and their first
derivatives are shown in order to find the phonon mode largely contribution to
the $\boldsymbol{\kappa}$ clearly. The effects of the isotope scattering and
boundary scattering are not included in them for simplicity. In the $\alpha$,
$\beta$, and $\gamma$ phases, the phonon modes upto $\sim$6, $\sim$12 and
$\sim$10 THz largely contribute to the $\boldsymbol{\kappa}$, respectively.  We
...
$\boldsymbol{\kappa}$, as mentioned in the previous section.  In
...
\end{align}
WDOS contains the information of both $\mathbf{v}_\lambda$ and number of heat
carriers.  Phonon linewidths are shown as scatter plots of $(\Gamma_\lambda,
\omega_\lambda)$, in Fig.~\ref{fig:Fig5_338_rev}-d.  $c_\lambda$ is approximately
$k_\rm{B}$ at 300 K and disregarded in our discussion. 

Firstly, we compare the properties between the $\alpha$ and $\beta$ phases.  
The linewidths and DOS are similar between the two phases and
$\kappa_{zz}^c/\kappa_{xx}^c$$\simeq$$h_{zz}/h_{xx}$ for each of the phases.
...
The $\gamma$ phase has its DOS, WDOS, and $\Gamma_\lambda$ distribution much
different from the others, consistently with the large differences in the
crystal structure. Some of the differences are remarked as follows: (1) The
first DOS peak is located at the higher frequency ($\sim$9 THz) than those in
the $\alpha$ and $\beta$ phases ($\sim$6 THz).  This is consistent with the band
diagram we have examined. At the lower frequencies than the peak, the DOS
intensities are the lowest among the three phases, indicating the lowest
distribution of heat carriers.  (2) In WDOS of $\omega \lesssim 10$ THz, the
$h_{xx}$ of the $\gamma$ phase has the next largest intensities to the $h_{zz}$
of the $\beta$ phase. This is explained by the large gradients of
$\omega_\lambda$ in the acoustic phonon branches. (3) In this $\omega$ range,
the $\gamma$ phase has approximately twice larger $\Gamma_\lambda$ than the
other phases. We will examine this details later.  As a result of these
characters, the $\kappa_{xx}$ is  not so large and the $\kappa^c_{xx}$ is
similar to that  of the $\beta$ phase.
...
Their formulae are described elsewhere.~\cite{phono3py} The weighting factors
reduce the $N_2^{(1)}$ near 0 THz and enhance the $N_2^{(2)}$ in the high
frequency range. The $N_2^{(1)}$ are dominant in the low frequency range of
$\omega\simeq$10 THz where
...
lower frequency range attribute to the large linewidths.  We set roughly the
frequency ranges for $\omega_\lambda$ so that the lower frequency range is the
range where the phonon modes largely contribute to the $\boldsymbol{\kappa}$. A
small change in the frequency ranges by a few THz did not change the qualitative
characters of the averages.  
