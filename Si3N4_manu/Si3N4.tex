...
$\Phi_{\lambda\lambda'\lambda''}$,\cite{phono3py} however, it is included in
...
\nabla_{\mathbf{q}}\omega_\lambda$ is the group velocity, and $c_\lambda$ is
...
terms in Eq.(\ref{eq:kappa}) characterize the behavior of the present
...
$\omega_\lambda$ and all indices in $\lambda'$ and $\lambda''$. To compare the
values of the structures with different $Z$, we multiply the average by
$(3n_a)^2$ where $n_a$ is the number of atoms in the primitive unit cell. The
averages are very similar for the $\alpha$ and $\beta$ phases. With the similar
impact of the WJDOS and $|\Phi_{\lambda\lambda'\lambda''}|^2$, the phonon
lifetimes in these phases are also similar. For the $\gamma$ phase, the large
