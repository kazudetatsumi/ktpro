...
largely depended on the microstructure of the samples:  \textcolor
...
gradients of $\omega_{\lambda}$ are large, reflecting the large bulk modulus
...
$\boldsymbol{\kappa}^c$ show the frequencies where the phonons largely
...
$v_{\lambda,i}^2$, we show the impacts of the $v_{\lambda,i}$ and the carrier
...
linewidths are also larger than the other phases in the frequency range. For
this point, we investigate the three-phonon-scattering strength $\Phi_{\lambda\lambda'\lambda''}$ 
shortly investigated here.  In Table.~\ref{table:aveavepp}, the magnitudes of
$\Phi_{-\lambda\lambda'\lambda''}$ are compared as the averages over two
frequency ranges of $\omega_\lambda$ and all ($\lambda'$,$\lambda''$). Its
...
opposite} impacts of the linewidth and WDOS, the $\kappa^c_{xx}$ qualitatively resembles
...
$v^2_{\lambda,i}$. The remaining property, linewidths are 
distributed in a similar way for both of the phases.  The DOS are also similar
...
Thus we investigate this further.
...
because $\Gamma_{\lambda}$ contains the delta functions corresponding to the
selection rule of the three-phonon scattering. 
...
auto-correlation function of DOS. The DOS for both of the $\alpha$ and $\beta$
phases show the frequency gap (Fig.~\ref{fig:Fig5_338_rev}-a).  $D_2^{(2)}$
...
$D_2^{(2)}$ shows a broad peak around 18 THz, which corresponds to the frequency
...
$\alpha$ and $\beta$ phases~\cite{kuwabara}, the major shapes of $D_2^{(2)}$,
reflecting this gap feature, are similar in these phases. With the same origin,
the JDOS of $D_2^{(1)}$ are also similar in these phases. 
As indicated in Table.~\ref{table:aveavepp}, $\Phi_{-\lambda\lambda'\lambda''}$
have similar impacts on the linewidths. 
With these similar impacts of
JDOS and $\Phi_{-\lambda\lambda'\lambda''}$ o n $\Gamma_\lambda$,
...
$\Gamma_\lambda$ below 5 THz are aligned on a single
...
along the $ab$ plane, which has 1 when the eigenvectors lie on the $ab$
plane. There is a tendency in the $\beta$ phase that  $\Gamma_\lambda$ are
large for the vibrations along the $ab$ plane. Therefore, within the
single-mode RTA, for the phonon modes below 5 THz, all of which belong to the
acoustic phonon branches, the vibration modes along the $ab$ plane are more
...
157 and 58, whose ratio is consistent to the population ratio of the vibration
modes along and out of the $ab$ plane.
