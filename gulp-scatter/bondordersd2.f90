...
!
!  Julian Gale, NRI, Curtin University, January 2014
!
  use datatypes
  use bondorderdata
...
!
  ...
  real(dp)                                       :: drlambdadrji
  real(dp)                                       :: drlambdadrjk
  real(dp)                                       :: rrij
  real(dp)                                       :: rrik
  real(dp)                                       :: rrjk
  real(dp)                                       :: rtmp
...
!  Calculate fik
!
                call mdftaper(rjk,rBOmin(npkj),rBOmax(npkj),fjk,dfjkdr,d2fjkdr2,d3fjkdr3,lgrad1,lgrad2,.false.)
!
!  Calculate Gijk
!
...
!
                ...
                if (lBOzrlR(nboRij)) then
!
!  Find repulsive bond order potential from j to k
!
                  lfound = .false.
                  nboRjk = 0
                  kk = neighno(k,j)
                  natk = nat(kk)
                  ntypk = nftype(kk)
                  do while (.not.lfound.and.nboRjk.lt.nboR)
                    nboRjk = nboRjk + 1
                    if (nBOspecR1(nboRjk).eq.natj.and.nBOspecR2(nboRjk).eq.natk) then
                      if ((nBOtypR1(nboRjk).eq.ntypj.or.nBOtypR1(nboRjk).eq.0).and. &
                          (nBOtypR2(nboRjk).eq.ntypk.or.nBOtypR2(nboRjk).eq.0)) then
                        lfound = .true.
                      endif
                    endif
                  enddo
                  if (.not.lfound) then
                    call outerror('no bond order found for j-k in ZRL algorithm',0_i4)
                    call stopnow('bondordersd2')
                  endif
                  rlambda = (BOlcoeffR(nboRji)*rij - BOlcoeffR(nboRjk)*rjk)
                else
                  rlambda = BOlcoeffR(nboRji)*(rij - rjk)
                endif
                expjik = exp(rlambda**mR)
!
!  Combine terms
!
...
!
                  ...
                  if (mR.ge.1) then
                    if (lBOzrlR(nboRij)) then
                      drlambdadrji = BOlcoeffR(nboRji)
                      drlambdadrjk = BOlcoeffR(nboRjk)
                    else
                      drlambdadrji = BOlcoeffR(nboRji)
                      drlambdadrjk = BOlcoeffR(nboRji)
                    endif
                    dexpjikdr = RmR*(rlambda**(mR-1))*expjik
                  else
                    dexpjikdr = 0.0_dp
                  endif
!
...
!
                  d1BtotjR(nj) = d1BtotjR(nj) + Gjik*fjk*dexpjikdr*rrij*drlambdadrji
                  d1BtotjR(k)  = d1BtotjR(k)  - Gjik*fjk*dexpjikdr*rrjk*drlambdadrjk
                  if (lgrad2) then
                    d2fjkdr2 = rrjk*rrjk*(d2fjkdr2 - dfjkdr)
                    if (mR.ge.2) then
                      d2expjikdr2 = expjik*(RmR*(RmR - 1.0_dp)*rlambda**(mR-2) + (RmR*rlambda**(mR-1))**2)
                    else
                      d2expjikdr2 = 0.0_dp
                    endif
!
                    nn = nj*(nj + 1)/2
                    d2BtotjR(nn) = d2BtotjR(nn) + Gjik*fjk*d2expjikdr2*rrij*rrij*drlambdadrji*drlambdadrji
                    d2BtotjR(nn) = d2BtotjR(nn) - Gjik*fjk*dexpjikdr*rrij*rrij*rrij*drlambdadrji
                    d2BtotjR(nn) = d2BtotjR(nn) + d2Gjikdr2(1)*fjk*expjik
                    d2BtotjR(nn) = d2BtotjR(nn) + 2.0_dp*dGjikdr(1)*fjk*dexpjikdr*rrij*drlambdadrji
!
                    if (nj.ge.k) then
                      nn = nj*(nj - 1)/2 + k
                    else
                    ...
                    d2BtotjR(nn) = d2BtotjR(nn) - Gjik*fjk*d2expjikdr2*rrij*rrjk*drlambdadrji*drlambdadrjk
                    d2BtotjR(nn) = d2BtotjR(nn) + Gjik*dfjkdr*dexpjikdr*rrij*drlambdadrji
                    d2BtotjR(nn) = d2BtotjR(nn) + d2Gjikdr2(2)*fjk*expjik
                    d2BtotjR(nn) = d2BtotjR(nn) + dGjikdr(1)*dfjkdr*expjik
                    d2BtotjR(nn) = d2BtotjR(nn) - dGjikdr(1)*fjk*dexpjikdr*rrjk*drlambdadrjk
                    d2BtotjR(nn) = d2BtotjR(nn) + dGjikdr(2)*fjk*dexpjikdr*rrij*drlambdadrji
!
                    if (nj.ge.nik) then
                      nn = nj*(nj - 1)/2 + nik
                    else
                    ...
                    d2BtotjR(nn) = d2BtotjR(nn) + dGjikdr(3)*fjk*dexpjikdr*rrij*drlambdadrji
!
                    nn = k*(k + 1)/2
                    d2BtotjR(nn) = d2BtotjR(nn) + Gjik*d2fjkdr2*expjik
                    d2BtotjR(nn) = d2BtotjR(nn) - 2.0_dp*Gjik*dfjkdr*dexpjikdr*rrjk*drlambdadrjk
                    d2BtotjR(nn) = d2BtotjR(nn) + Gjik*fjk*d2expjikdr2*rrjk*rrjk*drlambdadrjk*drlambdadrjk
                    d2BtotjR(nn) = d2BtotjR(nn) + Gjik*fjk*dexpjikdr*rrjk*rrjk*rrjk*drlambdadrjk
                    d2BtotjR(nn) = d2BtotjR(nn) + d2Gjikdr2(4)*fjk*expjik
                    d2BtotjR(nn) = d2BtotjR(nn) + 2.0_dp*dGjikdr(2)*dfjkdr*expjik
                    d2BtotjR(nn) = d2BtotjR(nn) - 2.0_dp*dGjikdr(2)*fjk*dexpjikdr*rrjk*drlambdadrjk
!
                    if (k.ge.nik) then
                      nn = k*(k - 1)/2 + nik
                    else
                    ...
                    d2BtotjR(nn) = d2BtotjR(nn) - dGjikdr(3)*fjk*dexpjikdr*rrjk*drlambdadrjk
!
                    nn = nik*(nik + 1)/2
...
!
                ...
                if (lBOzrlA(nboAij)) then
!
!  Find attractive bond order potential from j to k
!
                  lfound = .false.
                  nboAjk = 0
                  kk = neighno(k,j)
                  natk = nat(kk)
                  ntypk = nftype(kk)
                  do while (.not.lfound.and.nboAjk.lt.nboA)
                    nboAjk = nboAjk + 1
                    if (nBOspecA1(nboAjk).eq.natj.and.nBOspecA2(nboAjk).eq.natk) then
                      if ((nBOtypA1(nboAjk).eq.ntypj.or.nBOtypA1(nboAjk).eq.0).and. &
                          (nBOtypA2(nboAjk).eq.ntypk.or.nBOtypA2(nboAjk).eq.0)) then
                        lfound = .true.
                      endif
                    endif
                  enddo
                  ...
                  rlambda = (BOlcoeffA(nboAji)*rij - BOlcoeffA(nboAjk)*rjk)
                else
                  rlambda = BOlcoeffA(nboAji)*(rij - rjk)
                endif
                expjik = exp(rlambda**mA)
!
!  Combine terms
!
...
!
                  ...
                  if (mA.ge.1) then
                    if (lBOzrlA(nboAij)) then
                      drlambdadrji = BOlcoeffA(nboAji)
                      drlambdadrjk = BOlcoeffA(nboAjk)
                    else
                      drlambdadrji = BOlcoeffA(nboAji)
                      drlambdadrjk = BOlcoeffA(nboAji)
                    endif
                    dexpjikdr = RmA*(rlambda**(mA-1))*expjik
                  else
                    dexpjikdr = 0.0_dp
                  endif
!
...
!
                  d1BtotjA(nj) = d1BtotjA(nj) + Gjik*fjk*dexpjikdr*rrij*drlambdadrji
                  d1BtotjA(k)  = d1BtotjA(k)  - Gjik*fjk*dexpjikdr*rrjk*drlambdadrjk
                  if (lgrad2) then
                    d2fjkdr2 = rrjk*rrjk*(d2fjkdr2 - dfjkdr)
                    if (mA.ge.2) then
                      d2expjikdr2 = expjik*(RmA*(RmA - 1.0_dp)*rlambda**(mA-2) + (RmA*rlambda**(mA-1))**2)
                    else
                      d2expjikdr2 = 0.0_dp
                    endif
!
                    nn = nj*(nj + 1)/2
                    d2BtotjA(nn) = d2BtotjA(nn) + Gjik*fjk*d2expjikdr2*rrij*rrij*drlambdadrji*drlambdadrji
                    d2BtotjA(nn) = d2BtotjA(nn) - Gjik*fjk*dexpjikdr*rrij*rrij*rrij*drlambdadrji
                    d2BtotjA(nn) = d2BtotjA(nn) + d2Gjikdr2(1)*fjk*expjik
                    d2BtotjA(nn) = d2BtotjA(nn) + 2.0_dp*dGjikdr(1)*fjk*dexpjikdr*rrij*drlambdadrji
!
                    if (nj.ge.k) then
                      nn = nj*(nj - 1)/2 + k
                    else
                    ...
                    d2BtotjA(nn) = d2BtotjA(nn) - Gjik*fjk*d2expjikdr2*rrij*rrjk*drlambdadrji*drlambdadrjk
                    d2BtotjA(nn) = d2BtotjA(nn) + Gjik*dfjkdr*dexpjikdr*rrij*drlambdadrji
                    d2BtotjA(nn) = d2BtotjA(nn) + d2Gjikdr2(2)*fjk*expjik
                    d2BtotjA(nn) = d2BtotjA(nn) + dGjikdr(1)*dfjkdr*expjik
                    d2BtotjA(nn) = d2BtotjA(nn) - dGjikdr(1)*fjk*dexpjikdr*rrjk*drlambdadrjk
                    d2BtotjA(nn) = d2BtotjA(nn) + dGjikdr(2)*fjk*dexpjikdr*rrij*drlambdadrji
!
                    if (nj.ge.nik) then
                      nn = nj*(nj - 1)/2 + nik
                    else
                    ...
                    d2BtotjA(nn) = d2BtotjA(nn) + dGjikdr(3)*fjk*dexpjikdr*rrij*drlambdadrji
!
                    nn = k*(k + 1)/2
                    d2BtotjA(nn) = d2BtotjA(nn) + Gjik*d2fjkdr2*expjik
                    d2BtotjA(nn) = d2BtotjA(nn) - 2.0_dp*Gjik*dfjkdr*dexpjikdr*rrjk*drlambdadrjk
                    d2BtotjA(nn) = d2BtotjA(nn) + Gjik*fjk*d2expjikdr2*rrjk*rrjk*drlambdadrjk*drlambdadrjk
                    d2BtotjA(nn) = d2BtotjA(nn) + Gjik*fjk*dexpjikdr*rrjk*rrjk*rrjk*drlambdadrjk
                    d2BtotjA(nn) = d2BtotjA(nn) + d2Gjikdr2(4)*fjk*expjik
                    d2BtotjA(nn) = d2BtotjA(nn) + 2.0_dp*dGjikdr(2)*dfjkdr*expjik
                    d2BtotjA(nn) = d2BtotjA(nn) - 2.0_dp*dGjikdr(2)*fjk*dexpjikdr*rrjk*drlambdadrjk
!
                    if (k.ge.nik) then
                      nn = k*(k - 1)/2 + nik
                    else
                    ...
                    d2BtotjA(nn) = d2BtotjA(nn) - dGjikdr(3)*fjk*dexpjikdr*rrjk*drlambdadrjk
!
                    nn = nik*(nik + 1)/2
