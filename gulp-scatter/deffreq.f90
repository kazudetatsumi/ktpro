...
!
  use configurations, only : lbsmat, ramandir, ramandirtype
  use constants
  use control
  use current
  use defects
...
!
  ...
  if (imode.eq.2) then
    ...
    if (status/=0) call outofmemory('deffreq','raman')
  endif
!
...
!*********************************************
    if (linten) then
      if (imode.eq.2) then
        ...
        if (status/=0) call outofmemory('deffreq','nphonatptr')
        allocate(oscstrength(3,3,mcv),stat=status)
        if (status/=0) call outofmemory('deffreq','oscstrength')
!
!  Setup dummy nphonatptr
!
        do i = 1,numat
          nphonatptr(i) = i
        enddo
...
!
        call oscillatorstrengthg(mcv,numat,nphonatptr,ncfoc,iocptr,vectors,maxd2,oscstrength)
        if (lraman) then
          allocate(ramstrength(3,3,mcv),stat=status)
          if (status/=0) call outofmemory('deffreq','ramstrength')
!
!  Calculate Raman strengths
!
          call ramanstrength(mcv,numat,nphonatptr,ncfoc,iocptr,vectors,maxd2,ramstrength)
!
...
!  
      if (loldinten.or.imode.eq.1) then
!
!  Old algorithm - no use of oscillator strengths
!
        do i = 1,ncfoc
          w3(i) = 0.0_dp
          if (.not.lraman.and.imode.eq.2) then
            ix = 3*(i-1) + 1
            iy = ix + 1
            iz = ix + 2
            w2(ix) = 0.0_dp
...
!
          ...
          do j = 1,ncfoc
            ...
            if (.not.lraman.and.imode.eq.2) then
              raman(i) = raman(i) + w2(ind+1)*vectors(ind+1,i) + w2(ind+2)*vectors(ind+2,i) + w2(ind+3)*vectors(ind+3,i)
            endif
            ind = ind + 3
          enddo
...
!
            if (abs(rkt).gt.1.0d-12) then
              ffact = 1.0_dp+1.0_dp/(exp(abs(freq(i,1))*cmfact)-1.0_dp)
            else
              ffact = 1.0_dp
            endif
            if (abs(freq(i,1)).gt.1.0_dp) then
              ffact = ffact*1000.0_dp/abs(freq(i,1))
            else
              ffact = 0.0_dp
            endif
...
!
          ...
          do j = 1,ncfoc
            ...
            ind = ind + 3
          enddo
          w1(i)  = oscstrength(1,1,i) + oscstrength(2,2,i) + oscstrength(3,3,i)
...
!
!  Free memory that is specific to intensity calculation
!
      if (imode.eq.2) then
        if (lraman) then
          deallocate(ramstrength,stat=status)
          if (status/=0) call deallocate_error('deffreq','ramstrength')
        endif
        deallocate(oscstrength,stat=status)
        if (status/=0) call deallocate_error('deffreq','oscstrength')
        deallocate(nphonatptr,stat=status)
        if (status/=0) call deallocate_error('deffreq','nphonatptr')
      endif
    endif
!************************
!  Output eigenvectors  *
...
!
  if (imode.eq.2) then
    ...
    if (status/=0) call deallocate_error('deffreq','raman')
  endif
