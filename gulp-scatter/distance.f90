...
!   5/12 Format statements modified to allow for larger atom numbers.
!   3/14 Allocatable arrays changed for pointers
!
!  Conditions of use:
!
...
!
  ...
  integer(i4)                                  :: ntype
  integer(i4)                                  :: numt
  integer(i4), dimension(:), pointer,     save :: numtype => null()
  integer(i4)                                  :: status
  logical                                      :: lfout
  real(dp)                                     :: cut2
  real(dp)                                     :: diff
  ...
  real(dp)                                     :: rmin
  real(dp)                                     :: rtmp
  real(dp),    dimension(:), pointer,     save :: rtype
  real(dp)                                     :: xcd
  real(dp)                                     :: ycd
  real(dp)                                     :: zcd
  real(dp)                                     :: xcdi
...
!
  ...
  cut2 = cutb*cutb
  call realloc(numtype,maxtype,status)
  if (status.ne.0) call outofmemory('distance','numtype')
  call realloc(rtype,maxtype,status)
  if (status.ne.0) call outofmemory('distance','rtype')
!
!  Output header
!
