...
!
    ...
    if (status/=0) call outofmemory('property12','dpacked')
    allocate(ipivot(n),stat=status)
    if (status/=0) call outofmemory('property12','ipivot')
    allocate(wrk(3*n),stat=status)
    if (status/=0) call outofmemory('property12','wrk')
!
...
!
    ...
    if (status/=0) call deallocate_error('property12','wrk')
    deallocate(ipivot,stat=status)
    if (status/=0) call deallocate_error('property12','ipivot')
    deallocate(dpacked,stat=status)
    if (status/=0) call deallocate_error('property12','dpacked')
!
    if (ifail.ne.0) then
      nwarn = nwarn + 1
