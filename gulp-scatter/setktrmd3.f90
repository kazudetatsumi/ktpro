  subroutine setktrmd3(nkp,nkvec0,trm6,trm0,trm20,trm60,trm62,trm620,ldogammaonly)
!
!  Calculates the reciprocal space terms needed for the third derivatives
!
...
!   9/13 Option to force routine to do gamma point only added
!        This is used by the Raman case. 
...
!
  ...
  logical,     intent(in)  :: ldogammaonly             ! If true, then force gamma point only calculation without referencing nkp
...
!
  ...
  if (ldogammaonly) then
!
!  Set k point as gamma
!
    xkv = 0.0_dp
    ykv = 0.0_dp
    zkv = 0.0_dp
    ...
    if (ndim.eq.3) then
      ...
      if (.not.lgamma) then
        do i = 1,nkvec0
          ...
          rrk2 = 1.0_dp/rk2
          trm0(i) = xpon*vol4pi*rrk2*angstoev
          if (lstr) trm20(i) = -2.0_dp*trm0(i)*(4.0_dp*eta+rk2)*eta4*rrk2
        enddo
      endif
    elseif (ndim.eq.2) then
      do i = 1,nkvec
