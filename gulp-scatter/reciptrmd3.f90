...
!
      if (ndim.eq.3) then
        ...
        d3rs(2,1,4) = d3rs(2,1,4) + coss*tmp6*tmp4 + 0.5_dp*cosq*tmp5
        d3rs(2,2,4) = d3rs(2,2,4) + coss*tmp2*tmp4 + cosq*tmp4
        d3rs(3,1,4) = d3rs(3,1,4) + coss*tmp5*tmp4 + 0.5_dp*cosq*tmp6
        ...
        d3rs(2,1,5) = d3rs(2,1,5) + coss*tmp6*tmp5 + 0.5_dp*cosq*tmp4
        d3rs(2,2,5) = d3rs(2,2,5) + coss*tmp2*tmp5
        d3rs(3,1,5) = d3rs(3,1,5) + coss*tmp5*tmp5 + 0.5*cosq*(tmp1 + tmp3)
        d3rs(3,2,5) = d3rs(3,2,5) + coss*tmp4*tmp5 + 0.5_dp*cosq*tmp6
        ...
        d3rs(3,1,6) = d3rs(3,1,6) + coss*tmp5*tmp6 + 0.5_dp*cosq*tmp4
        d3rs(3,2,6) = d3rs(3,2,6) + coss*tmp4*tmp6 + 0.5_dp*cosq*tmp5
...
!
        ...
        d3is(2,1,4) = d3is(2,1,4) - sins*tmp6*tmp4 - 0.5_dp*sinq*tmp5
        d3is(2,2,4) = d3is(2,2,4) - sins*tmp2*tmp4 - 0.5_dp*sinq*tmp4
        d3is(3,1,4) = d3is(3,1,4) - sins*tmp5*tmp4 - 0.5_dp*sinq*tmp6
        d3is(3,2,4) = d3is(3,2,4) - sins*tmp4*tmp4 - 0.5_dp*sinq*(tmp2 + tmp3)
        d3is(3,3,4) = d3is(3,3,4) - sins*tmp3*tmp4 - sinq*tmp4
        d3is(1,1,5) = d3is(1,1,5) - sins*tmp1*tmp5 - sinq*tmp5
        d3is(2,1,5) = d3is(2,1,5) - sins*tmp6*tmp5 - 0.5_dp*sinq*tmp4
        d3is(2,2,5) = d3is(2,2,5) - sins*tmp2*tmp5
        d3is(3,1,5) = d3is(3,1,5) - sins*tmp5*tmp5 - 0.5_dp*sinq*(tmp1 + tmp3)
        d3is(3,2,5) = d3is(3,2,5) - sins*tmp4*tmp5 - 0.5_dp*sinq*tmp6
        d3is(3,3,5) = d3is(3,3,5) - sins*tmp3*tmp5 - sinq*tmp5
        d3is(1,1,6) = d3is(1,1,6) - sins*tmp1*tmp6 - sinq*tmp6
        d3is(2,1,6) = d3is(2,1,6) - sins*tmp6*tmp6 - 0.5_dp*sinq*(tmp1 + tmp2)
        d3is(2,2,6) = d3is(2,2,6) - sins*tmp2*tmp6 - sinq*tmp6
        d3is(3,1,6) = d3is(3,1,6) - sins*tmp5*tmp6 - 0.5_dp*sinq*tmp4
        d3is(3,2,6) = d3is(3,2,6) - sins*tmp4*tmp6 - 0.5_dp*sinq*tmp5
        d3is(3,3,6) = d3is(3,3,6) - sins*tmp3*tmp6
      elseif (ndim.eq.2) then
        d3rs(3,1,1) = d3rs(3,1,1) + sinsz*tmp1*xrkk + 2.0_dp*sinr*ztrm1*xrkk
        ...
        d3rs(3,2,2) = d3rs(3,2,2) + sinsz*tmp2*yrkk + 2.0_dp*sinr*ztrm1*yrkk
...
!
        ...
        d3rs(2,2,6) = d3rs(2,2,6) + coss*tmp2*tmp6 + 2.0_dp*cosq*tmp6
!
        d3is(3,1,1) = d3is(3,1,1) + cossz*tmp1*xrkk + 2.0_dp*cosr*ztrm1*xrkk
        ...
        d3is(3,2,2) = d3is(3,2,2) + cossz*tmp2*yrkk + 2.0_dp*cosr*ztrm1*yrkk
...
! 3 z components
      ...
      if (lstr) then
        if (ndim.eq.3) then
          ...
          d3s(2,1,4) = d3s(2,1,4) + 0.5_dp*cosq*tmp5
          d3s(3,1,4) = d3s(3,1,4) + 0.5_dp*cosq*tmp6
          d3s(2,2,4) = d3s(2,2,4) + cosq*tmp4
          d3s(3,2,4) = d3s(3,2,4) + 0.5_dp*cosq*(tmp2 + tmp3)
          ...
          d3s(2,1,5) = d3s(2,1,5) + 0.5_dp*cosq*tmp4
          d3s(3,1,5) = d3s(3,1,5) + 0.5_dp*cosq*(tmp1 + tmp3)
          d3s(3,2,5) = d3s(3,2,5) + 0.5_dp*cosq*tmp6
          ...
          d3s(2,1,6) = d3s(2,1,6) + 0.5_dp*cosq*(tmp1 + tmp2)
          d3s(3,1,6) = d3s(3,1,6) + 0.5_dp*cosq*tmp4
          d3s(2,2,6) = d3s(2,2,6) + cosq*tmp6
          d3s(3,2,6) = d3s(3,2,6) + 0.5_dp*cosq*tmp5
        elseif (ndim.eq.2) then
          d3s(1,1,1) = d3s(1,1,1) + (coss*tmp1 + 3.0_dp*cosq)*tmp1
          d3s(2,1,1) = d3s(2,1,1) + (coss*tmp1 + 2.0_dp*cosq)*tmp6
          d3s(2,2,1) = d3s(2,2,1) + (coss*tmp1 + cosq)*tmp2
          d3s(1,1,2) = d3s(1,1,2) + (coss*tmp2 + cosq)*tmp1
          d3s(2,1,2) = d3s(2,1,2) + (coss*tmp2 + 2.0_dp*cosq)*tmp6
          d3s(2,2,2) = d3s(2,2,2) + (coss*tmp2 + 3.0_dp*cosq)*tmp2
          d3s(3,1,1) = d3s(3,1,1) + (sinsz*tmp1 + 2.0_dp*sinr*ztrm1)*xrkk
          ...
          d3s(3,2,2) = d3s(3,2,2) + (sinsz*tmp2 + 2.0_dp*sinr*ztrm1)*yrkk
