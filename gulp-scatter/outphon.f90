...
!   8/13 Modified to allow for imaginary modes appearing in the DOS
!  10/13 Gaussian broadening added as an option
!  10/13 Modified so that the renormalisation of functions is not 
!        performed, given that the functions include normalisation 
!        constants. 
!
!  Conditions of use:
!
  ...
  do i = 1,nlk
    do j = 1,mint
      if (lkptdispersion(i+nlkpt-1)) then
        ...
        if (freq(j,i).lt.fmindisp) fmindisp = freq(j,i)
      else
        ndenpoints = ndenpoints + 1
        if (freq(j,i).gt.fmaxdos) fmaxdos = freq(j,i)
        if (freq(j,i).lt.fmindos) fmindos = freq(j,i)
      endif
    enddo
  enddo
!
...
!
  if (lbroad) then
    if (lbroaden_gauss) then
      fextra = sqrt( - log(sqrt(pi)/(1000.0_dp*bfactor))/bfactor**2)
    else
  ...
  fmindos = fmindos - fextra
!
!  If imaginary phonons are not to be output then reset fmindos/fmindisp
!
  if (.not.linclude_imaginary) then
    fmindisp = 0.0_dp
    fmindos = 0.0_dp
  endif
!
...
!
  ...
  if (nbox.eq.0.and.fbox.gt.0.0_dp) then
    lfboxzero = .false.
    nbox = ((fmaxdos-fmindos)/fbox) + 1
  elseif (nbox.gt.0.and.fbox.eq.0.0_dp) then
    lnboxzero = .false.
    fbox = (fmaxdos-fmindos)/dble(nbox) + 1.0d-6  ! Add a small amount to fbox to prevent rounding error leading to fmax lying outside nbox range
  endif
...
!
        ...
        if (frq.gt.0.0_dp.or.linclude_imaginary) then
          if (lbroad) then
!****************
!  Broaden DOS  *
!****************
            ...
            enddo
            do k = 1,nbox
              wght(k) = wght(k) + wt*wghtb(k)
            enddo
          else
!*******************
!  Delta function  *
!*******************
            ibox = ((frq-fmindos)/fbox) + 1
...
!
  if (ldenout) then
    ...
    if (lbroad) then
      if (lbroaden_gauss) then
        write(ioout,'(''  Gaussian broadening factor = '',f10.4,/)') bfactor
      else
...
!
        if (nlbox.eq.0.and.flbox.eq.0.0_dp) then
          nlbox = 25
          flbox = ((fmaxdisp+10.0_dp-fmindisp))/float(nlbox)
        elseif (nlbox.eq.0) then
          nlbox = ((fmaxdisp-fmindisp)/flbox) + 1
...
!
          ...
          do l = 1,nhkpt
            do ll = 1,mint
              nptl = ((freq(ll,indf+l)-fmindisp)/flbox) + 1
              if (nptl.eq.j) then
                istep1 = nint(l*rstep)
