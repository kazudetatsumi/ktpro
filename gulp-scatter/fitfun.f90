...
!
    tim1 = g_cpu_time()
    lfirst = .true.
    lfgrad = (lfcborn(nc).or.lfcprop(nc).or.lfcphon(nc).or.lfcscatter(nc))
...
!***********************
    ...
    if (lfcscatter(nc)) then
      call scatter(.false.,.true.,fsumsq_scatter)
    endif
    lsymderv2 = lsymd2save
