...
!**********
  ...
  if (abs(angstoev-14.3997584_dp).gt.1.0d-7) then
    write(iout,'(''units angstoev '',f18.14)') angstoev
  endif
!***************************
...
!******************************
  ...
  if (llammpspots) then
    if (lammpspotsfile(1:1).ne.' ') then
      write(iout,'(''output lammps_pots '',f8.4,1x,f8.4,1x,i8,1x,a60)') lammps_r0,lammps_rend,nlammpspoints,lammpspotsfile(1:60)
    else
  ...
  if (lcas) then
    if (casfile(1:1).ne.' ') then
      write(iout,'(''output castep_phon '',a60)') casfile(1:60)
    else
  ...
  if (lshengBTE) then
    if (casfile(1:1).ne.' ') then
      write(iout,'(''output shengBTE '',a60)') shengfile(1:60)
    else
...
!************************
  if (lfastMPI) then
    write(iout,'(''parallel communicate'')')
  endif
!
!  Free local memory
!
