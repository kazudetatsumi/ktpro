...
!   8/13 access replaced by position in append statement
!
!  Conditions of use:
!
...
!
!  Julian Gale, NRI, Curtin University, August 2013
!
...
!
  if (larc.and.lmovie.and.ioproc) then
    if (mdafil(1:1).ne.' ') then
      open(32,file=mdafil,status='old',position='append',form='formatted',err=103)
      goto 105
    else
      open(32,status='old',position='append',form='formatted',err=103)
      goto 105
    endif
103 continue
!
...
!
  if (lhis.and.ioproc) then
    if (hisfile(1:1).ne.' ') then
      open(34,file=hisfile,status='old',position='append',form='formatted',err=107)
      goto 110
    else
      open(34,status='old',position='append',form='formatted',err=107)
      goto 110
    endif
107 continue
!
...
!
  if (lpre.and.ioproc) then
    if (prefile(1:1).ne.' ') then
      open(35,file=prefile,status='old',position='append',form='formatted',err=113)
      goto 120
    else
      open(35,status='old',position='append',form='formatted',err=113)
      goto 120
    endif
113 continue
    if (prefile(1:1).ne.' ') then
...
!
  if (ldcd.and.ioproc) then
    if (dcdfile(1:1).ne.' ') then
      open(36,file=dcdfile,status='old',form='unformatted',position='append',err=130)
    endif
    goto 135
130 continue
!
