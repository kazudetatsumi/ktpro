...
!
    if (abs(yy).lt.1.0d-12) yy = sign(1.0_dp,yy)
    if (abs(ys).lt.1.0d-12) ys = sign(1.0_dp,ys)
!
    do I = 1,N
      diag(I) = YS/YY
    enddo
...
!
!      INFO = 7  Appears to be stuck in cycle and so need to reset
!
...
!  Debugging output
!
  if (ioproc.and.index(keyword,'linmin').ne.0) then
    write(6,'(''    MCSRCH : Convergence tests '')')
    write(6,'(''      Bracket?  '',l2)') BRACKT
    write(6,'(''      Step?     '',3f12.6)') stp,stmin,stmax
    write(6,'(''      Function? '',2f12.6)') f,ftest1
    write(6,'(''      Gradient? '',2f12.6)') DG,dgtest
  endif
!
!  Trap case where something has gone wrong and the line search is stuck in an infinite loop
!
  fdiff = f - flast
  if (ncycle.gt.3.and.fdiff.gt.1.0d-5) INFO = 7
  flast = f
!
