...
!   8/13 Neurton scattering modifications merged back
...
!
  ...
  lscatterloc = (lscatter.and.ndim.eq.3)
  loptsuccess = .false.
!
...
!
  ...
  if (lmain) then
    ...
    if (.not.loptiloc) then
      ...
      if (lborn.or.lproploc.or.lphon.or.lscatterloc.or.lharmrelax) iflag = 2
      lstr = (ndim.gt.0)
      ifail = 4
    endif
...
!
    ...
    if (.not.lfree.and..not.loptiloc.and..not.lharmloc) then
      if ((lprop.or.lborn.or.lphon.or.lscatter).and.(lewald.or.lwolf)) then
...
!
      if (lscatterloc) then
        call scatter(.true.,.false.,fsumsq)
      endif
      if (lproploc.and.lshengBTE) then
!
!  Write output shengBTE control file
!
        call outshengBTEcontrol(32_i4)
      endif
...
!
    if (lmain.and..not.lharmloc) call optin
!
!  Start minimisation
!
...
!
      call harmonicrelax(nvar,1_i4,xc,fc,gc,hess,nhwords,1_i4,lfree)
      ifail = 4
      loptsuccess = .true.
...
!
    lfreeze = .false.
    if ((lborn.or.lproploc.or.lphonloc.or.lscatterloc).and.(loptiloc.or.lharmloc)) then
      lopt = .false.
      lstrold = lstr
      lstr = (ndim.eq.3)
      iflag = 2
...
!
      ...
      if (lproploc.and.ldefect.and.(lphonloc.or.lscatterloc)) then
!
!  If a defect calculation is to be performed do silent property calculation
!  to restore matrices after phonon calculation
!
...
!
  ...
  if (ioproc) then
    ...
    elseif (lmain.and.(lproploc.or.lphonloc.or.lscatterloc)) then
