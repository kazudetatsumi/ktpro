  subroutine fourimp12(eimp,lgrad1,lgrad2,imode,xderv,yderv,zderv)
!
!  Subroutine for four-body improper torsion energy of defects
!
...
!
!   7/13 Created from fourimp12 & fouroop12
!
!  Conditions of use:
!
...
!
  ...
  if (status/=0) call outofmemory('fourimp12','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourimp12','ntypmiddle')
  allocate(nptrnforimp(nfor),stat=status)
  if (status/=0) call outofmemory('fourimp12','nptrnforimp')
!
...
!           
            if (l.eq.k.or.l.eq.i) cycle lloop
!  
!  Molecularity check
!           
            lbondedkl = .false.
            if (lmol.and.lneedmol) then
              ...
              if (lmolok.and.lbtyp) then
                if (imode.eq.1) then
                  if (lregion1j.and.lregion1l) then
                    icm = 1
                    lbondedkl = .false.
                    do while (icm.le.nbondsdef(j).and..not.lbondedkl)
                      imm = nbondeddef(icm,j)
                      lbondedkl = (imm.eq.l)
                      icm = icm + 1
                    enddo
                  else
                    if (npsj.gt.0.and.npsl.gt.0) then
                      call mindtoijk(indml,lxx,lyy,lzz)
                      call mindtoijk(indmj,jxx,jyy,jzz)
                      lxx = lxx - jxx
                      lyy = lyy - jyy
                      ...
                      lyy = lyy + lyy2 - jyy2
                      lzz = lzz + lzz2 - jzz2
                      call bonded(lbondedkl,l2bondskl,nbtypekl,nbtypekl2,npsj,npsl,lxx,lyy,lzz)
                    else
                      lbondedkl = .false.
                    endif
                  endif
...
!
  ...
  if (status/=0) call deallocate_error('fourimp12','nptrnforimp')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimp12','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimp12','natmiddle')
!
  return
  end
