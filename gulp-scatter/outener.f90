...
!
  ...
  if (nfor.gt.0) then
    ...
    write(ioout,'(''  Improper torsions          = '',f20.8,'' eV'')') eimp
  ...
  if (lkim_model) then
    write(ioout,'(''  OpenKIM potentials         = '',f20.8,'' eV'')') ekim
  endif
  ...
  if (abs(erelax).gt.1d-8) then
    write(ioout,'(''  Harmonic relaxation energy = '',f20.8,'' eV'')') erelax
  endif
