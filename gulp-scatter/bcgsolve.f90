  subroutine bcgsolve(n,nloc,nlocptr,A,maxnumA,numA,nAdiag,listA,b,x,tol,itmax,iter,err)
...
!
  ...
  real(dp),    intent(in)    :: A(maxnumA,nloc)    ! Values of non-zero elements of column of A
...
!
  ...
  call sparseAdiagprecon(n,nloc,nlocptr,A,maxnumA,nAdiag,r,z)
!
! Main loop
!
  iter = 0
  ...
  do while (iter.le.itmax.and..not.converged)
    iter = iter + 1
    call sparseAdiagprecon(n,nloc,nlocptr,A,maxnumA,nAdiag,rr,zz)
...
!
  ...
  subroutine sparseAdiagprecon(n,nloc,nlocptr,A,maxnumA,nAdiag,Vin,Vout)
!
