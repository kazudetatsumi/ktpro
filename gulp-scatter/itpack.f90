  subroutine jcg(n,ndim,maxnz,jcoef,coef,rhs,u,iwksp,nw,wksp,iparm,rparm,ier)
!
...
!
  ...
  integer(i4) :: jcoef(ndim,maxnz),iwksp(1),iparm(12)
  real(dp)    :: coef(ndim,maxnz),rhs(n),u(n),wksp(nw),rparm(12) 
!
  integer(i4) :: i, nb, nbo, n3, loop, ib1, ib2, ib3, ib4, ib5, idgts
  real(dp)    :: g_cpu_time, digit1, digit2
  real(dp)    :: temp, time2
  real(dp)    :: time1, timi1, timi2, timj1, timj2, tol
!
  external perror
!
...
10 format (///1x,'i t p a c k      j c g      ')
  ...
  if (iparm(11).eq.0) timj1 = g_cpu_time()
...
220 if (iparm(11).eq.0) timi1 = g_cpu_time()
!
...
!
  ...
  integer(i4) :: jcoef(ndim,maxnz)
  real(dp)    :: u(n),u1(n),d(n),d1(n),dtwd(n),tri(2,1),coef(ndim,maxnz)
!
...
!
  ...
  integer(i4) :: jcoef(ndim,maxnz),iparm(12)
  real(dp)    :: coef(ndim,maxnz),rhs(n),rparm(12)
!
  integer(i4) :: i, j
!
!
...
!
  integer(i4) :: n, ndim, maxnz
  integer(i4) :: jcoef(ndim,maxnz),p(n),iwork(n)
  real(dp)    :: coef(ndim,maxnz),work(n)
!
  integer(i4) :: i, j
!
!
...
!
  ...
  real(dp)    :: coef(ndim,maxnz),rhs(n),u(n),work(2*n)
!
  integer(i4) :: i
...
!
  ...
  integer(i4) :: jcoef(ndim,maxnz),p(n),ip(n)
!
! ... specifications for local variables
!
...
!
  ...
  integer(i4) :: jcoef(ndim,maxnz)
  real(dp)    :: tol
  real(dp)    :: coef(ndim,maxnz),rhs(n),work(n)
!
  integer(i4) :: i, j, jcol
!
...
!
  ...
  integer(i4) :: jcoef(ndim,maxnz)
  real(dp)    :: coef(ndim,maxnz),rhs(n),u(n),work(n)
!
