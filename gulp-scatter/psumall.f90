...
!
  ...
  allocate(sum(max(29,8*n+1+nstrains*(numat+1))),stat=status)
  if (status/=0) call outofmemory('psumall','sum')
  allocate(sum0(max(29,8*n+1+nstrains*(numat+1))),stat=status)
...
!****************
  ...
  sum0(28) = eimp
  sum0(29) = ekim
  call sumall(sum0,sum,29_i4,"psumall","energies")
  eatom = sum(1)
  ereal = sum(2)
  erecip = sum(3)
  ec6 = sum(4)
