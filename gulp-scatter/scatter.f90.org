...
!
   do i = 1,3
     ...
     enddo
   enddo
   call matinv(qv,3_i4,3_i4,wrk,ifail)
!
!  FEEDBACK:  Output of all the options, flags and settings for this run
!
  if (lprintloc) then
    write(ioout,'(/,'' **********************************************************************'')')
    write(ioout,'('' SCATTER - Neutron Scattering Functionality for GULP'',/)')
    write(ioout,'('' ----------------------------------------------------------------------'')')
    write(ioout,'('' Daniel L. Roach  (University of Salford, 2010) '')')
    write(ioout,'('' Please reference the following paper when using this code - '')')
    write(ioout,'('' Roach DL, Ross DK and Gale JD, Phys Rev B (2010) xxxx,xxxx,xxx '')')
    write(ioout,'('' **********************************************************************'',/)')
    write(ioout,'('' Calculation Parameters and Settings :'')')
    write(ioout,'('' -------------------------------------'',/)')
! 
!  k_type flags:  Type of kernel used
!
    write(ioout,'('' Kernel selection: '')')
    write(ioout,'('' -----------------'',/)')
    if (q_caltype.eq.1) then
      write(ioout,'('' Coherent Inelastic scan enabled '',/)')
    elseif (q_caltype.eq.2) then
      write(ioout,'('' Incoherent Inelastic scan enabled '',/)')
    elseif (q_caltype.eq.3) then
      write(ioout,'('' Total inelastic scan enabled '',/)')
    elseif (q_caltype.eq.4) then
      write(ioout,'('' Complete inelastic scan (with all contributions flagged) enabled '',/)')
    elseif (q_caltype.eq.5) then
      write(ioout,'('' Sampling-only run:  Sampling of (Q,w) space will be written to file. '',/)')
    endif
!
!  s_type flags:  Type of sampling used
!
    write(ioout,'('' Sampling method: '')')
    write(ioout,'('' ----------------'',/)')
    if (q_runtype.eq.1) then
      write(ioout,'('' Polycrystalline sampling using rso method enabled'',/)')
      write(ioout,'(/,'' Range of Q values = '',f10.4,'' to '',f10.4,'' Inv. Angs in '',i6,'' steps'')') &
        q_qmin,q_qmax,nq_step
    elseif (q_runtype.eq.2) then
      write(ioout,'('' Linear sampling using vector-set method enabled ...'')')
      write(ioout,'('' Note that all vectors are fractional.'',/)')
      write(ioout,'('' Number of sampling vectors = '',i4,/)') n_qvector
      do i = 1,n_qvector
        write(ioout,'('' Vector '',i4,'' : '',f10.4,2x,f10.4,2x,f10.4)') &
              i, q_ordinate(1,i), q_ordinate(2,i), q_ordinate(3,i)
        write(ioout,'(12x,''to '',f10.4,''  '',f10.4,''  '',f10.4,'' in '',i4,'' steps.'',/)') &
          q_ordinate(4,i), q_ordinate(5,i), q_ordinate(6,i),n_qpoint(i)      
      enddo
    elseif (q_runtype.eq.3) then
      write(ioout,'('' Polycrystalline sampling using cylindrical sampling method enabled '',/)')
!      write(ioout,'(/,'' Range of Q values = '',f10.4,'' to '',f10.4,'' Inv. Angs in '',i6,'' steps'')') &
!      q_qmin,q_qmax,nq_step
    elseif (q_runtype.eq.4) then
      write(ioout,'('' Polycrystalline sampling using Monkhorse-Pack method enabled '',/)')
    elseif (q_runtype.eq.5) then
      write(ioout,'('' Polycrystalline sampling using Monte Carlo method enabled '',/)')
    elseif (q_runtype.eq.6) then
      if ((abs(q_ordinate(4,1))+abs(q_ordinate(5,1))+abs(q_ordinate(6,1))).eq.0.0_dp) then
        call outerror('h k l equal zero in scattering direction',0_i4)
        call stopnow('scatter')
      endif
      write(ioout,'('' Linear sampling using single vector (from Gamma) method enabled '',/)')
      write(ioout,'('' Direction of Q (h k l fractional) = '',3f10.4)') &
                        q_ordinate(4,1),q_ordinate(5,1),q_ordinate(6,1) 
      write(ioout,'(/,'' Number of sampled points over this vector = '',i6)') n_qpoint(1)
      lscatsurfaceweight = .true.
    elseif (q_runtype.eq.7) then
      write(ioout,'('' Sampling input from existing file. '',/)')
    endif
!
!  s_options flags: Scattering correction flags and fitting flag options  
!
    if (lscatoptflag) then
      write(ioout,'('' Scattering correction and/or fitting options enabled: '',/)')
      if (lscatopt(1)) then
        write(ioout,'('' Dynamic Debye Waller function enabled: '',/)')
      elseif (lscatopt(2)) then
        write(ioout,'('' Library-sourced Debye Waller function enabled: '',/)')
      elseif (lscatopt(3)) then
        write(ioout,'('' User-defined Debye Waller function enabled: '',/)')
      elseif (lscatopt(4)) then
        write(ioout,'('' Multi-phonon scattering correction function enabled: '',/)')
      elseif (lscatopt(5)) then
        write(ioout,'('' Multiple scattering correction function enabled: '',/)')
      elseif (lscatopt(6)) then
        write(ioout,'('' Least-squares fitting function enabled: '',/)')
      elseif (lscatopt(7)) then
        write(ioout,'('' GEO fitting function enabled: '',/)')
      elseif (lscatopt(8)) then
        write(ioout,'('' GSM fitting function enabled: '',/)')
      endif
    endif
!
!  s_analysis:  Pre-analysis and output options
!
    if (lscatanoptflag) then
      write(ioout,'('' Pre-analysis output options enabled: '',/)')
      if (lscatanopt(1)) then
        write(ioout,'('' Single axis slice options enabled: '',/)')
      elseif (lscatanopt(2)) then
        write(ioout,'('' Instrument loci output options enabled '',/)')
      elseif (lscatanopt(3)) then
        write(ioout,'('' Threshold comparison function enabled '',/)')
      elseif (lscatanopt(4)) then
        write(ioout,'('' Subtract multi-phonon scattering contribution from main scan: '',/)')
      elseif (lscatanopt(5)) then
        write(ioout,'('' Subtract multiple scattering contribution from main scan '',/)')
      elseif (lscatanopt(6)) then
        write(ioout,'('' Subtract debye-waller contribution from main scan '',/)')
      elseif (lscatanopt(7)) then
        write(ioout,'('' Use GEO matching algorithm '',/)')
      endif
    endif
!
!  e_transfer:   Energy transfer output options
!
    write(ioout,'('' Minimum omega value = '',f10.4,'' cm-1'')') q_wmin
    write(ioout,'('' Maximum omega value = '',f10.4,'' cm-1'')') q_wmax
    if (q_runtype .eq. 6) then
      write(ioout,'(/,'' Main S(Q,w) will be output as '',a30,''.sxy'')') trim(adjustl(q_filename))
    else
      write(ioout,'(/,'' Main S(Q,w) will be output as '',a30,''.sqw'')') trim(adjustl(q_filename))
    endif
    write(ioout,'(/)')
  endif !basic output info (ioproc)
