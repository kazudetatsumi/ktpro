...
!
  use constants,      only : avogadro, evtoj, boltz, kjmtoev
  ...
  use parallel
  use partial
  use shell,          only : ncore, nshell, nshptr
...
!
    ...
    if (status/=0) call outofmemory('property0','dpacked')
    allocate(ipivot(n),stat=status)
    if (status/=0) call outofmemory('property0','ipivot')
    allocate(wrk(3*n),stat=status)
    if (status/=0) call outofmemory('property0','wrk')
!
...
!
    ...
    if (status/=0) call deallocate_error('property0','wrk')
    deallocate(ipivot,stat=status)
    if (status/=0) call deallocate_error('property0','ipivot')
    deallocate(dpacked,stat=status)
    if (status/=0) call deallocate_error('property0','dpacked')
!
    if (ifail.ne.0) then
      nwarn = nwarn + 1
