  subroutine fourimpp(xkv,ykv,zkv)
!
!  Subroutine for four-body improper torsion phased second derivatives
!
!   7/13 Created from fourp
!  11/13 Condition on upper half triangular k only removed as this is incorrect
!
!  Conditions of use:
!
...
!
!  Julian Gale, NRI, Curtin University, November 2013
!
  use constants
  use current
  use derivatives
...
!
  ...
  if (status/=0) call outofmemory('fourimpp','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourimpp','ntypmiddle')
  allocate(nptrnforimp(nfor),stat=status)
  if (status/=0) call outofmemory('fourimpp','nptrnforimp')
  allocate(nuniqueiptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimpp','nuniqueiptr')
  allocate(nuniquekptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimpp','nuniquekptr')
  allocate(nuniquelptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimpp','nuniquelptr')
!
...
!
  ...
  if (status/=0) call deallocate_error('fourimpp','nuniquelptr')
  deallocate(nuniquekptr,stat=status)
  if (status/=0) call deallocate_error('fourimpp','nuniquekptr')
  deallocate(nuniqueiptr,stat=status)
  if (status/=0) call deallocate_error('fourimpp','nuniqueiptr')
  deallocate(nptrnforimp,stat=status)
  if (status/=0) call deallocate_error('fourimpp','nptrnforimp')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimpp','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimpp','natmiddle')
!
  return
  end
