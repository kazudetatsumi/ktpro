...
!
  ...
  use iochannels
  use kim_models,    only : lkim_model, lkim_model_cfg_OK, nkimmodel
  use kspace
  use m_three
  use molecule
  use one,           only : none
...
!*******************
  if (lkim_model) then
    do nkim = 1,nkimmodel
      if (lkim_model_cfg_OK(nkim,ncf)) then
        ekimloc = 0.0_dp
!        if (lsymderv2) then
!          call kimsd2(nkim,ekim,lgrad1,lgrad2)
!        else
!          if (lgrad2) then
!            call kim(nkim,ekim,0.0_dp,0.0_dp,0.0_dp,lgrad1,lgrad2,.false.)
!          else
            call kimmd(nkim,ekim,lgrad1)
!          endif
!        endif
        ekim = ekim + ekimloc
      endif
...
!********************************
  if (nprocs.gt.1) then
    call psumall(eatom,ereal,erecip,ec6,eqeq,eattach,esregion12,esregion2,ethb,efor, &
      ...
      efield,eradial,ereaxFF,eplane,ecosmo,eone,eedip,eimp,ekim,lgrad1,lsymderv)
  endif
!
