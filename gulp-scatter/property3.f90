...
!
      ...
      if (status/=0) call outofmemory('property3','dpacked')
      allocate(ipivot(n),stat=status)
      if (status/=0) call outofmemory('property3','ipivot')
      allocate(wrk(3*n),stat=status)
      if (status/=0) call outofmemory('property3','wrk')
!
...
!
      ...
      if (status/=0) call deallocate_error('property3','wrk')
      deallocate(ipivot,stat=status)
      if (status/=0) call deallocate_error('property3','ipivot')
      deallocate(dpacked,stat=status)
      if (status/=0) call deallocate_error('property3','dpacked')
!
      if (ifail.ne.0) then
        nwarn = nwarn + 1
