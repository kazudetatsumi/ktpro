...
!
  ...
  if (reg1last(ncf).gt.0.0_dp) then
...
!
  ...
  if (mode2a.ge.3) then
    if ((reg2(ncf)-rdcmax).lt.(rpmax-1.0d-8).and.abs(qdef).gt.0.0_dp) then
      nwarn = nwarn + 1
    ...
    if (reg2(ncf).lt.rpmax.and.abs(qdef).gt.0.0_dp) then
      nwarn = nwarn + 1
      write(ioout,'(''  **** Warning - radius of region 1 + 2a is too small for region 2b energy ****'')')
