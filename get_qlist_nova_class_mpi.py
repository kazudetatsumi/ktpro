...
class get_qlist:
    ...
    def spect(self, qmin, qmax):
        ...
        rz1d = np.zeros((esize//psize))
        #comm.Barrier()
        for eidx in range(rank*esize//psize, (rank+1)*esize//psize):
            cond = np.where(np.abs(y-ene[eidx]) < 0.0000000001)
            rz1d[eidx-rank*esize//psize] = np.sum(z[cond])
        #comm.Barrier()
        comm.Allgather(rz1d, z1d)
        self.ene = ene
        self.spectra = z1d
        #self.create_fig()
        #plt.plot(ene, z1d)
