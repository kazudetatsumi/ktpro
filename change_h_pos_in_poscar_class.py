...
class change_hpos():
        ...
        for ix in range(0, self.nx):
            for iy in range(0, self.nx):
                for iz in range(0, self.nx):
                    hpos.append([self.std - self.edgelength/2 + ix*dx,
                                 self.std - self.edgelength/2 + iy*dx,
                                 self.std - self.edgelength/2 + iz*dx])
        self.hpos = np.array(hpos)

    def GetIrreducibleShift_old(self):
        irr_hpos = self.hpos[0].reshape((1, 3))
        for _hpos in self.hpos:
    ...
    def GetIrreducibleShift(self):
        ...
        for rot, trans in zip(self.sym['rotations'][1:],
            ...
            sym_hpos = (np.matmul(irr_hpos, rot) + trans) % 1.0
            cond = np.ones((sym_hpos.shape[0]), dtype=bool)
            for i, _sym_hpos in enumerate(sym_hpos):
                if cond[i]:
                    cond *= np.sum(np.abs(((irr_hpos - _sym_hpos) % 1.0)),
                                   axis=1) >= 1e-5
                    cond[i] = True
    ...
    def GetDataOverAllHpos2(self):
        ...
        for ih, _hpos in enumerate(self.hpos):
            ...
            for rot, trans in zip(self.sym['rotations'],
                ...
                cond *= np.sum(np.abs((_hpos - sym_irr_hpos) % 1.0), axis=1) >= 1e-5
            self.irr_idx[ih] = np.arange(0, self.irr_hpos.shape[0])[np.invert(cond)]
    ...
    def GetPotential(self):
        self.potential = self.ene[self.irr_idx].reshape((self.nx, self.nx, self.nx))

    def PlotPotential(self):
        #plt.pcolor(self.potential[:, :, 6] - np.min(self.potential))
        #plt.colorbar()
        plt.plot(self.potential[:, 6, 6] - np.min(self.potential))
        plt.show()
