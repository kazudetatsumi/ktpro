...
import subprocess
from ctypes import *
lib = CDLL("/home/kazu/ktpro/isefortsearch.so")
...
def calc_ise_search_f90(data, condition, pdfs, maxw, sumpdf):
    ...
    print("---check the calculated ise function---")
    opt_indx = np.unravel_index(np.argmin(ise, axis=None), ise.shape)
    opt_indx = (opt_indx[0] + 1, opt_indx[1] + 1, opt_indx[2] + 1,
                opt_indx[3] + 1)
    print("opt_indx for ise", opt_indx,
          "with the ise = ",
          ise[opt_indx[0] - 1, opt_indx[1] - 1, opt_indx[2] - 1,
              opt_indx[3] - 1])
