...
# Reused for user interface of bin width optimization routine which works on
# BL01,14.
# FY2023 Kazuyoshi TASTSUMI
...
class Plot_4w_Logn:
    def __init__(self, infile, deltas, m, islog=False, isplain=True, msize=80):
        self.infile = infile
        self.deltas = deltas
        self.m = m
        self.islog = islog
    ...
    def plotter(self, clm=1, ylabel=True):
        self.get_data()
        ...
        for ip, dp in enumerate(self.deltas):
            ...
            if self.islog:
                ...
                x_m = 1.0/self.list_m[0]
                y_m = self.list_m[1+ip]*dp
            ax = self.fig.add_subplot(4, 1, clm+ip)
            if self.isplain:
                ax.scatter(x_n, y_n)
                ...
                ax.scatter(x_n[self.m-1:self.m], y_n[self.m-1:self.m],
                           ...
                           facecolors="white")
                ax.scatter(x_m[self.m:ymm], y_m[self.m:ymm],
                           marker='.', edgecolor="black", s=self.msize,
                           facecolors="black")
                ...
                ax.plot(x_m[0:ymm], y_m[0:ymm],
                        marker='None', color="k", linestyle="dotted")
            ax.tick_params(labelbottom=False)
            ax.set_ylim(0, max([np.max(y_n), np.max(y_m)]) + dp)
            if self.islog:
                ax.set_xscale('log')
            ...
            ax.set_xlabel('count')
            ax.yaxis.set_major_locator(MultipleLocator(dp))
