...
contains
  ...
  function ise4d(nw3, nw2, nw1, nw0, datasize3, datasize2, datasize1, datasize0,& 
	...
	double precision, allocatable :: ssumpdfs(:), sise(:)
	...
	mrank = psize - rank -1
	if (histsize(1) >= mrank+1) then
	  !do hx = 1, histsize(1)
      do hx = mrank+1, histsize(1) - mod(histsize(1)-mrank-1, psize), psize
	  do hy = 1, histsize(2)
	  do hz = 1, histsize(3)
	  do hw = 1, histsize(4)
	    if (hist_cond(hx, hy, hz, hw) == maxcond) then
	      sumpdfs = sumpdfs + sum(pdfs((hx-1)*nw(1)+1:hx*nw(1),(hy-1)*nw(2)+1:hy*nw(2), &
		                         (hz-1)*nw(3)+1:hz*nw(3),(hw-1)*nw(4)+1:hw*nw(4)))
	    endif
	  enddo
  	  enddo
  	  enddo
	...
	call mpi_allgather(sumpdfs, 1, mpi_double_precision, ssumpdfs, 1, mpi_double_precision,  mpi_comm_world, ierr)
	sumpdfs = sum(ssumpdfs)
	...
	call mpi_allgather(ise, 1, mpi_double_precision, sise, 1, mpi_double_precision,  mpi_comm_world, ierr)
	ise = sum(sise) / (product(nw)*sumhist)
	ise4d%isepon = ise
