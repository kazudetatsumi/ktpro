...
from scipy.sparse.linalg._eigen.arpack.arpack import ArpackNoConvergence  # 収束例外
...
def get_min_bond_length(h_frac: np.ndarray,
                        axis: np.ndarray,
...
def get_V(Gs: np.ndarray, nx: int, z: np.ndarray):
    """Build V in the G basis with reduced intermediate memory."""
...
#    """Return Hψ matvec in the G basis (修正版：実空間で V を掛ける)."""
#    gmod = (Gs % nx).astype(int)
#    gidx = (gmod[:, 0], gmod[:, 1], gmod[:, 2])
#
#    # ★ ここがポイント：z を実空間へ戻す（= V/EH；平均差し引き済）
#    V_r = np.fft.ifftn(z)                     # complex128 のままでOK
#
#    def mv(x: np.ndarray) -> np.ndarray:
#        X = np.zeros((nx, nx, nx), dtype=np.complex128)
#        X[gidx] = x
#        phi_r = np.fft.ifftn(X)              # 実空間の波動関数
#        Y_r = V_r * phi_r                  # 実空間での掛け算 ← 本来の物理演算
#        Y_full = np.fft.fftn(Y_r)             # 逆空間へ戻す
...
#    E = E[idx]
#    U = U[:, idx]
#    print('Krylov (eigsh) finished: k=', k, ' min(E)[meV] =', np.min(E))
...
class NQEKrylovSolver:
    """
    A small utility to solve Hψ using the Krylov (eigsh) method.
    - Multiply V/EH in real space and transform back by FFT (safe & consistent)
    - Stores fixed data (gidx, nx, Kdiag, V_r) inside self
    - solve() performs sorting (ascending order) and enforces .real for safety
    """
    ...
    def __init__(self,
        ...
        # G → indices on the FFT grid (fixed)
        gmod = (Gs % nx).astype(int)
        self.gidx = (gmod[:, 0], gmod[:, 1], gmod[:, 2])
        self.Ng = gmod.shape[0]

        # Dimensionless potential in real space: V_r = (V3d - mean)/Eh
        if subtract_mean:
            self.V_r = (np.asarray(V3d, float) - float(np.mean(V3d))) / Eh
        else:
            self.V_r = np.asarray(V3d, float) / Eh

        # Diagonal of Kinetic term
        self.Kdiag = np.asarray(Kdiag, dtype=dtype)

        # Working buffer (reduces per-call memory allocations)
        self._X = np.zeros((nx, nx, nx), dtype=dtype)

        # Prepare LinerOperator, which can be put by .operator)
        self.operator = LinearOperator(
            shape=(self.Ng, self.Ng),
            matvec=self._matvec,
            dtype=dtype
        )

    # ---- matvec of Hψ, multiplying V in real space ----
    def _matvec(self, x: np.ndarray) -> np.ndarray:
        X = self._X
        X.fill(0.0)
        X[self.gidx] = x.astype(self.dtype, copy=False)

        # Transform to real space → multiply by V → transform
        phi_r = np.fft.ifftn(X)
        Y_r = self.V_r * phi_r
        Y_f = np.fft.fftn(Y_r)

        # Extract G slots + apply Kdiag·x
        y = Y_f[self.gidx] + self.Kdiag * x
        return y.astype(self.dtype, copy=False)

    # ---- Optional quick self-check for Hermitian property ----
    def check_hermitian(self, ntry: int = 3, tol: float = 1e-10) -> bool:
        for _ in range(ntry):
            x = (np.random.randn(self.Ng)
                 + 1j*np.random.randn(self.Ng)).astype(self.dtype)
            y = (np.random.randn(self.Ng)
                 + 1j*np.random.randn(self.Ng)).astype(self.dtype)
            Hx, Hy = self._matvec(x), self._matvec(y)
            lhs = np.vdot(x, Hy)  # <x|H|y>
            rhs = np.vdot(Hx, y)  # <Hx|y> = <x|H†|y>
            if not np.allclose(lhs, rhs, atol=tol, rtol=0):
                return False
        return True

    # ---- Solve the lowest k eigenpairs using Krylov (with sorting) ---
    def solve(self,
              *,
              k: int = 30,
              which: str = "SA",
        ...
        """ Calculate k eigen pairs and return (E_meV, U) """
        # Safety adjustment for k / ncv
        k_eff = max(1, min(k, self.Ng - 2))     # Supposing Ng>=3
        if ncv is None:
        ...
        # Execute eigsh or fall back in exception
        try:
            E_h, U = eigsh(self.operator, k=k_eff, which=which, tol=tol,
                           maxiter=maxiter, ncv=ncv_eff, v0=v0_c)
        except ArpackNoConvergence:
            if verbose:
                print("[WARN] eigsh did not converge;\
                        retry with relaxed tol/ncv")
        ...
        # Sorting: ascending order + convert to real (for safety)
        ...
        if verbose:
            print(f"Krylov (eigsh) finished:\
                    k={k_eff}, min(E)[meV]={E_meV[0]:.10f}")
