    ...
    def fit(self, X, num_xy=list(), channel_vals=list(), unit_name='Channel'):
        ...
        for rep in range(self.reps):
            ...
            for j in range(self.n_components):
                ...
                C[:, j] = C[:, j] / (np.sqrt(np.dot(C[:, j].T , C[:, j])) + 1e-16)
            cj = np.sum(C, axis=1)
            ...
            for itr in range(self.max_itr):
                ...
                for j in range(self.n_components):
                    ...
                    S[:, j] = XC[:, j] - np.dot(S , C2[:, j]) + C2[j, j] * S[:, j]
                    if self.flag_nonneg:
    ...
    def plot_spectra(self, figsize=list(), normalize=True):
        ...
        for k in range(self.S_.shape[1]):
            if normalize:
                ...
                self.S_[:, k] = self.S_[:, k] / (np.sqrt(np.dot(self.S_[:, k].T , self.S_[:, k])) + 1e-16)
        ...
        for rep in range(self.reps):
            ...
            for itr in range(self.max_itr):
                ...
                for j in range(self.n_components):
                    ...
                    C[:, j] = C[:, j] - self.wo * (np.dot(cj.T , C[:, j])) / (np.dot(cj.T , cj)) * cj
                    C[:, j] = (C[:, j] + np.abs(C[:, j])) / 2  # replace negative values with zeros
                    #C[:, j] = C[:, j] / (np.sqrt(C[:, j].T @ C[:, j]))  # normalize
                    C[:, j] = C[:, j] / (np.sqrt(np.dot(C[:, j].T , C[:, j])))  # normalize
                    cj = cj + C[:, j]
    ...
    def fit(self, X, num_xy=list(), channel_vals=list(), unit_name='Channel'):
        ...
        for rep in range(self.reps):
            ...
            for j in range(self.n_components):
                ...
                c = (np.sqrt(np.dot(S[:, j].T , S[:, j])))  # normalize
                if c > 0:
                    S[:, j] = S[:, j] / c
                else:
            ...
            for itr in range(self.max_itr):
                ...
                obj[itr] = obj[itr] + np.dot((L ** (-1)).T , (np.sum(C, axis=0) + self.beta_).T) \
        ...
        X_est = np.dot(self.C_ , self.S_.T)  # reconstructed data matrix
