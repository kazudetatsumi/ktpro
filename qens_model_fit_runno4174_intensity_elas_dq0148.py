...
class qens_model_fit(qbr):
    ...
    def plotter(self, fig, nr, x, y, t, e, mask, title, sidx, cidx):
        ...
        ax.text(1.6, -0.7, title+"_"+str(self.runNos[cidx]))
        ax.text(1.6, -1.2, '$u^2$={:.2f} +/- {:.2f} '.format(self.u2[sidx, cidx], self.stdu2[sidx, cidx]) + '$\AA^2$')
        ax.text(1.6, -1.6, 'rel. error +/- {:.1f}% '.format(self.stdu2[sidx, cidx]/np.abs(self.u2[sidx, cidx])*100.))
        ax.set_ylim(-5., -0.)
        #ax.set_ylim(0, 400.)
        #ax.set_xlim(0., 1.6)
