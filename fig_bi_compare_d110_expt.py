...
def compare_images4():
    fdir = "/home/kazu/restormer_rev2_lim/bi3d/restormer_conv3d/for_single/" +\
            "train/211/true_edge/nll/gau2ch/ktrand/applied_on_exptdata/"
    data_name = ['denoisedx2_noisy_5models', 'denoised_noisy_5models',
                 'expt_noisy', 'expt']
    ...
    bitest[6:] = np.array(((timage[1, :6] - timage[0, :6])**2 +
                           timage[1, 6:]**2)**0.5)
    cond = ((bitest[4] - bitest[10]) < x_test[4]) &\
           ((bitest[4] + bitest[10]) > x_test[4])
...
def plote(data, target, pred, datae, targete, prede, savefig='dummy',
    ...
    vmaxim = np.max([np.unique(np.sort(target))[-25],
                     np.unique(np.sort(data))[-25],
                     np.unique(np.sort(pred))[-25]])
    vminim = np.min([np.unique(np.sort(target))[15],
                     np.unique(np.sort(data))[15],
                     np.unique(np.sort(pred))[15]])
    for didx, (d, label) in enumerate(zip([target, data, pred], [labels[1],
                                          labels[0], labels[2]])):
        _d = copy.deepcopy(d)
        _d[_d == 0.] = np.nan
        ...
        ax[0, didx].tick_params(axis='both')
    time_idx = [int(t*data.shape[0]) for t in time_idx]
    q_idx = [int(t*data.shape[1]) for t in q_idx]
    tidx = time_idx[1]
    qidx = q_idx[1]
    cond = np.where(data[tidx, :qidx] > 0.)[0]
    ymin1 = np.min(data[tidx, :qidx][cond]-datae[tidx, :qidx][cond])-0.0003
    ymax1 = np.max(data[tidx, :qidx][cond]+datae[tidx, :qidx][cond])+0.0003
    cond = np.where(data[:tidx, qidx] > 0.)[0]
    ymin2 = np.min(data[:tidx, qidx][cond]-datae[:tidx, qidx][cond])-0.0003
    ymax2 = np.max(data[:tidx, qidx][cond]+datae[:tidx, qidx][cond])+0.0003
    margin = 3
    dshape = target.shape
    ...
    for didx, (d, de, label, c) in enumerate(
            zip([data[tidx, :qidx], pred[tidx, :qidx]],
                [datae[tidx, :qidx], prede[tidx, :qidx]], [labels[0],
                labels[2]], ['gray', 'k'])):
    ...
    for didx, (d, de, label, c) in enumerate(
        ...
        ax[2, didx+1].plot(cond, target[:, qidx][cond], label=labels[1], c='k',
                           ls='--')
        cond = np.where(d > 0.)[0]
