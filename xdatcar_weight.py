...
# Modified by K. Tatsumi
# mass weighted VAF & unweighted PDOS with Bose distribution func(at average Temp)
# 2020/03/16
...
class xdatcar:
    ...
    def getVAF(self):
        ...
        #        self.VAF2 += np.correlate(self.velocity[:,i,j],
                self.VAF2 += self.mions[i]*np.correlate(self.velocity[:,i,j],
                                          self.velocity[:,i,j], 
                                          'full')
        # only hydrogen atom
        #for j in range(3):
        #    self.VAF2 += np.correlate(self.velocity[:,12,j],
        #                                  self.velocity[:,12,j], 
        #                                  'full')
        # only hydrogen atom along z 
        #self.VAF2 = np.correlate(self.velocity[:,12,2],
        #                                  self.velocity[:,12,2], 
        #                                  'full')
        # two-sided VAF
