...
class qens_model_fit(qbr):
    ...
    def plotter(self, fig, nr, x, y, t, e, mask, title, sidx, cidx):
        ...
        ax.text(0.3, -2.6, title+"_"+str(self.runNos[cidx]))
        ax.text(0.3, -3.1, '$u^2$={:.2f} +/- {:.2f} '.format(self.u2[sidx, cidx], self.stdu2[sidx, cidx]) + '$\AA^2$')
        ax.text(0.3, -3.5, 'rel. error +/- {:.1f}% '.format(self.stdu2[sidx, cidx]/np.abs(self.u2[sidx, cidx])*100.))
