...
def run():
    ...
    plotranges = np.array([[-0.675, 2.8], [-0.925, 3.8], [-0.8, 0.60],
                          [0, 36.4]])
    devs = np.array([1, 1, 1])
    ...
    pro = c4c.CROSS(outfile, orthotope_lims, wholeranges, devs, cpos,
                    hvlofs=False, binwidths=binwidths, showorthob=False,
                    plotranges=plotranges, vivid=True)
...
#    pro.plot_crosssection(3)
    plt.savefig("fig_adv_hist_with_labels_only4.png")
