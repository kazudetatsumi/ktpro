...
# mpi version of get_ise_auto.py
...
def search_ns_in_resultfile(infile, tcount):
    ...
    while extraFound == 0:
        ...
        if line.startswith('extrapolation'):
            ...
            values = line.split()
            if abs(tcount - float(values[0]))/tcount <= 0.001:
                nsfound = 1
...
def get_ise(workdir):
    ...
    mrank = size - rank - 1
    if histsize[0] >= mrank + 1:
        for hx in range(mrank+ 0, histsize[0] - ((histsize[0]-mrank-1) % size) + size - 1, size):
    ...
    sumpdfs = comm.allgather(sumpdfs)
    if rank == 0:
        print("sumpdfs is shared as an array")
        print(time.time() - start)
                                ...
                                for py in range(hy*ns[1], (hy+1)*ns[1]):
                                    for pz in range(hz*ns[2], (hz+1)*ns[2]):
                                        for pw in range(hw*ns[3], (hw+1)*ns[3]):
                                            ise += (pdfs[px, py, pz, pw] - datahist[hx, hy, hz, hw]/(1.0*sumdata/sum(sumpdfs)*sumpdf*np.prod(ns)))**2
    ise = ise / (np.prod(ns)*sumhist*1.0)
    ise = comm.allgather(ise)
    #print("average_pdfs:",sumpdfs/((np.prod(ns)*sumhist*1.0)))
    #print("ise",ise)
    #print("sq_ise/average_pdf (%)", ise**0.5/(sumpdfs/((np.prod(ns)*sumhist*1.0)))*100)
    if rank == 0:
        print(np.sum(datahist),sum(ise),sum(ise)**0.5/(sum(sumpdfs)/(np.prod(ns)*sumhist*1.0))*100)
