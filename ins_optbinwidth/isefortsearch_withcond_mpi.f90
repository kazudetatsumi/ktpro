...
!large volume version : includes bins which has at least one unmasked initial array element.
!Kazuyoshi TATSUMI 2021/03/11
...
contains
  ...
  subroutine cost4d(comm, maxnw3, maxnw2, maxnw1, maxnw0, datasize3, datasize2, datasize1, datasize0,& 
    condparam, sumpdf, data_array, pdfs, condition, ise) bind(C)
    ...
    double precision, intent(in) :: condparam, sumpdf
	...
	sumdata = sum(pack(hist_array, hist_cond >= condparam*maxcond))
	sumhist = count(hist_cond >= condparam*maxcond)
	do hx = 1, histsize(1)
	do hy = 1, histsize(2)
	do hz = 1, histsize(3)
	do hw = 1, histsize(4)
	  if (hist_cond(hx, hy, hz, hw) >= condparam*maxcond) then
