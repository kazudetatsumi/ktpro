...
fig.suptitle("crosssections of 4D INS phantom data for Ei24", fontsize="x-large")
Ei = 23.65 # [meV]
...
def get_pdf(omega, gamma, de,  ddscs):
    ...
    ene = np.linspace(-2.0, 21.12, int((21.12 + 2.0) // de) + 1)
    ...
    for eidx in range(0, pdf.shape[3]):
        pdf[:, :, :, eidx] = gaussian_filter(pdf[:, :, :, eidx], sigma=[2.875/2.3548/(2.0)**0.5, 2.875/2.3548, 2.875/2.3548/(2.0)**0.5])
...
def fun_gaus(x, mean):
    sigma = eres_ama(mean) / 2.3548
    dx = x[1] - x[0]
    #l = 1.0 / (sigma * (2.0*np.pi)**0.5) * np.exp(-0.5 *
    #                                              ((x - mean) / sigma)**2)
    #l = l/(np.sum(l)*dx)
    l = dx / (sigma * (2.0*np.pi)**0.5) * np.exp(-0.5 *
                                                 ((x - mean) / sigma)**2)
    return l
...
def eres_siki(mean):
    ...
    freq = 300  # Chopper frequency [Hz]
    Ei = 23.65  # Incident Neutron Energy [meV]
...
def plotter(xi, xe, yi, ye, lx, ly, data, vn, hn, cn, dev, text):
    ...
    print("fuck",np.max(data[xi:xe, yi:ye])/dev)
    ...
    ax.text(2, 10, text, color='white')
    ax.axis('tight')


def plot_crosssection_more_useful(xi, xe, yi, ye, zi, ze, ei, ee, condition, data4):
    plotter(xi, xe, yi, ye, 'qx', 'qy', np.sum(data4[:, :, :, 25], axis=2),     3, 2, 1, 1.0, "qz:all, E=40")
    plotter(xi, xe, yi, ye, 'qx', 'qy', np.sum(condition[:, :, :, 25], axis=2), 3, 2, 2, 1.0, "")
    plotter(yi, ye, ei, ee, 'qy', 'E',  data4[74, :, 8, :],                   3, 2, 3, 1.0, "qx=175, qz=16")
    plotter(yi, ye, ei, ee, 'qy', 'E',  condition[74, :, 8, :],               3, 2, 4, 1.0, "")
    plotter(xi, xe, ei, ee, 'qx', 'E',  data4[:, 58, 8, :],                    3, 2, 5, 1.0, "qy=77, qz=16")
    #plotter(xi, xe, ei, ee, 'qx', 'E',  condition[:, 77, 16, :],                3, 2, 6, 1.0, "")
    plotter(xi, xe, ei, ee, 'qx', 'E',  condition[:, 58, 8, :],                3, 2, 6, 1.0, "")
...
def run():
    ...
    gamma = 0.8                           # FWHM [meV]
    dE = 0.08                             # [meV]
    qpointsfile = "/home/kazu/WORK/vasp-phonopy/cu_BL14_Ei24/dq0.02/qpoints.hdf5"
    ddscsfile = "/home/kazu/WORK/vasp-phonopy/cu_BL14_Ei24/dq0.02/ddscs.hdf5"
    ...
    ei = 121
    ee = 289
    condition = read_h5py(maskfile, "condition")
    save_pdf_hdf5("pdf.hdf5", pdf[:, :, :, :], condition[:, :, :, :])
    plot_crosssection_more_useful(xi, xe, yi, ye, zi, ze, ei, ee, condition*1.0, pdf)
    plt.savefig("pdf.png")
