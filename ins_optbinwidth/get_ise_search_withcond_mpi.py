...
lib = CDLL("/home/kazu/ktpro/isefortsearch_withcond_mpi.so")
...
def calc_ise_search_f90(data, condition, pdfs, maxw, condparam, sumpdf):
...
def preconditioning(rootdir, condparam):
    data4 = read_h5py(rootdir+"try1/0.5m/eliminated_data.hdf5", "data4")*1.0
    condition = read_h5py(rootdir+"try1/0.5m/eliminated_data.hdf5", "condition")
    condition = np.array(condition, dtype="int32")
    # This is a stricter version of the search range.
    #maxnw = [data4.shape[0]//2, data4.shape[1]//2, data4.shape[2]//2, data4.shape[3]//2]
    maxnw_data = [data4.shape[0]//2, data4.shape[1]//2, data4.shape[2]//2, data4.shape[3]//2]
    maxnw = getbinidx2(rootdir+"try1/0.5m/std-0.5m.log")*3
    maxnw = [min(maxnw[i], maxnw_data[i]) for i in range(0,4)]
    #maxnw = [2, 2, 2, 2]
    print(maxnw)
    pdf = read_h5py(rootdir + "pdf.hdf5", "pdf")
    sumpdf = np.sum(pdf)
    pdfs = np.array(pdf[xi:xe, yi:ye, zi:ze, ei:ee])
    calc_ise_search_f90(data4, condition, pdfs, maxnw, condparam, sumpdf)
    #ise, avepdfs, fracise = calc_ise_search_f90(data4, condition, pdfs, maxnw, sumpdf)
    #return np.sum(data4), ise, avepdfs, fracise


def run():
    rootdir = "/home/kazu/desktop/200312/for_cu_new/old_orthotope/" +\
    ...
    preconditioning(rootdir, condparam)
