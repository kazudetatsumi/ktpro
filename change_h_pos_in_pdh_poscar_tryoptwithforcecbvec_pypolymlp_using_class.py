...
def getene_wstr(hposs, positions, case):
    ene = []
    case.structures[0].positions[:, 15] = positions
    for hpos in hposs:
...
def get_E0(positions, prj, case):
    ...
    if np.max(np.abs(positions - prj.positions[15, :])) > 1e-15 or 'E_comp' not in dir(prj):
        print('**get_E0: processes from potential to GetEigen are to be done')
        prj.positions[15, :] = positions[:]
...
def samplerun():
    ...
    positions[15, 0] += -0.01112663224481034296*2.5
    positions[15, 1] += 0.02112663224481034296*2.5
    positions[15, 2] += -0.03112663224481034296*2.5
