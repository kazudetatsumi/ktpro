...
levelslow = np.flip(0.1**np.arange(1, 15))
levelshigh = np.flip(0.5**np.arange(3))
levelsveryhigh = 2**np.arange(1, 10)
levels = np.concatenate((levelslow, levelshigh, levelsveryhigh))
fig, ax = plt.subplots(1, 3)
#ax[0].pcolor(X, Y, f_kde)
ax[0].contour(X, Y, f_kde, levels)
ax[0].set_aspect('equal')
ax[0].set_title('Estimated using gaussian kde')
#ax[1].pcolor(X, Y, f_cop)
ax[1].contour(X, Y, f_cop, levels)
ax[1].set_title('Estimated using copulad')
ax[1].set_aspect('equal')
#ax[2].pcolor(X, Y, Z)
ax[2].contour(X, Y, Z, levels)
