...
def run():
    ...
    logfiles = []
    for run, temp in zip(runs, temps):
    ...
    for hidx in range(0, 2):
        [k0, k1], R2 = optimize(1./temps, np.log(hwhm[:, hidx]), variables=[0.0, -1.0])
        x = np.linspace(np.min(1./temps), np.max(1./temps), 100)
        plt.scatter(1./temps, np.log(hwhm[:, hidx]), label=r'$\Gamma$'+str(hidx))
        plt.plot(x, k0 + k1*x)
        plt.text(0.00355, -5.-hidx*2,  "$R^2$={:.3f}".format(R2))
    ...
    for ilog, logfile in enumerate(logfiles):
        with open(logfile, 'r') as f:
            ...
            for il, line in enumerate(lines):
                if '[' == line[0] and "using" in lines[il-1] and "cov" in lines[il+1]:
                    if ' ' != line[1]:
                        hwhm[ilog, 0] = line.split()[1]
                        hwhm[ilog, 1] = line.split()[3]
                    else:
                        hwhm[ilog, 0] = line.split( )[2]
                        hwhm[ilog, 1] = line.split( )[4]
    return(hwhm)
